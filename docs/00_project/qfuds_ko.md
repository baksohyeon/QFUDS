...
- [docs/wiki/fiction/](../wiki/fiction/)은 별도 취미 트랙입니다. 핵심은 소설 결과물이
  아니라, 픽션 작가를 위한 에이전틱 하네스입니다. 세계관을 IP처럼 관리하고, 연속성,
  검토, 초안 관리 워크플로를 실험합니다. 물리 증거도 연구 결과도 아닙니다.
...
논문 형식으로 압축해 둔 보관본입니다. 처음 읽을 문서는 아닙니다. 실제 입구는 003과
004입니다.

## 실제로 남은 것

이 저장소가 보여주는 가장 강한 결과는 QFUDS가 확정 이론이 됐다는 것이 아닙니다.
그렇지 않습니다. 의미 있는 결과는 넓은 직관을 명시적 테스트로 밀어 넣었고, 무엇이
살아남았는지, 무엇이 기존 IV/IDE 언어로 환원됐는지, 어디서 현대 우주론의 미해결 난제에
막혔는지를 파일로 남겼다는 점입니다.

그래서 읽는 순서는 단순합니다. 먼저 에이전틱 연구 하네스를 보고, 검증된 본 줄기인 003,
그다음 거친 끝까지의 탐색인 004를 보면 됩니다. 005는 입문 문서가 아니라 논문형 보관본으로
두는 편이 맞습니다. fiction 영역은 별도 취미 트랙입니다. 픽션 작가용 에이전틱 워크플로와
