...
## 이 저장소 읽는 법: 시점이 다른 두 단계

이 프로젝트에는 성격이 다른 두 층이 있습니다. 지금 어느 층을 읽고 있는지에 따라
문장의 무게가 달라집니다. 두 층은 같은 시점에 나온 결과가 아니고, 검증 수준도 같지
않습니다.
...
논문 형식으로 압축한 선택 문서입니다. 바로 아래
[현재 위치](#현재-위치-벽까지-밀어붙여-실제로-알아낸-것) 절은 이 탐색이 어디까지
갔는지를 정리한 보고입니다. 증명이 아니라, 비전문가가 탐색 결과를 정리한 기록으로
읽어 주세요.

한 줄로: **1단계는 진짜 시험을 통과한 것(003까지), 2단계는 실패한 구현체를 그래도
벽까지 밀어붙였을 때 나온 것입니다. 대표 기록은 004이고, 005는 선택적으로 읽는
논문형 압축본입니다.**
...
이 값들이 실제로 학계가 쓰는 값과 정확히 맞아떨어졌습니다. 의미는 있었지만, 동시에
...
## 이건 어떻게 만들었나: 에이전틱 연구 시스템

앞의 물리 이야기는 결과의 한쪽입니다. 다른 한쪽은 그 결과를 만들고 걸러낸 작업
...
[lineage 006](../wiki/lineage/006_agentic_research_system_ko.md)에 있고, 그게 깨질
때마다 어떻게 고쳤는지 정직하게 적은 기록은
[회고 폴더](../wiki/postmortem/)(총 11개)에 있습니다.
...
- [006_agentic_research_system_ko.md](../wiki/lineage/006_agentic_research_system_ko.md): 에이전트 기반 연구 운영 절차 기록. 문서·상태 경계, 워크플로 라우팅, 자산 캐시 상태 사다리, PageIndex/MarkItDown MCP, 적대적 검토 패스, 결정론적 깃훅 게이트
