...
이 글은 먼저 세 가지 관점으로 시스템을 읽는 법을 짚고, 그 다음 전체 구조, 상태 기준점,
운영 워크플로, 문헌 자산 파이프라인, 실행 루프, 검증 게이트, 회고 규율 순서로 내려갑니다.
이 문서에서 말하는 운영 절차는 AI에게 더 많은 자유를 주기 위한 것이 아니라, 성급한 긍정
판단이나 논리 점프를 줄이기 위해 문서와 코드에 제한을 둔 방식입니다.

## 세 가지 관점으로 읽기

이 시스템은 보는 사람에 따라 다르게 읽힙니다. 같은 폴더, 같은 파일인데 역할이 바뀌면
의미가 달라집니다. 이 한 가지만 머리에 두면 나머지가 훨씬 쉽습니다.

비유로 보면, 같은 주방을 세 사람이 다르게 봅니다.

- **요리사(에이전트)** 는 "지금 무슨 순서로 움직여야 하나"를 봅니다.
- **손님(사람)** 은 "메뉴가 뭐고 내 주문이 어디까지 됐나"를 봅니다.
- **사장(팀)** 은 "여러 요리사가 같이 일할 때 규칙이 뭐고, 겹치면 어떻게 푸나"를 봅니다.

| 관점 | 한 줄 | 핵심 질문 |
| --- | --- | --- |
| 에이전트 관점 | "나는 이 파일들로 이렇게 행동한다." | 세션 시작 때 무엇을 읽고, 입력을 어디로 분류하고, 무엇을 검증하나? |
| 사람 관점 | "나는 이걸 이렇게 이해하고 쓰면 된다." | 어디에 무엇을 남기고, 무엇이 최신이고, 어디서 끊고 방향을 바꾸나? |
| 협업 관점 | "여러 명이 같이 쓰면 이렇게 된다." | 팀 규칙은 뭐고, 충돌이 왜 생기고 어떻게 풀며, 언제 검증하나? |

에이전트 관점에서 핵심은 **행동 순서**입니다. 에이전트는 [AGENTS.md](../../../AGENTS.md)로
금지선과 태도를 읽고, [로드맵](../../05_next_steps/000_roadmap.md)으로 현재 상태를 확인하고,
[.agent/workflows](../../../.agent/workflows/)에서 작업 절차를 고릅니다. 그 다음 문서 위치를
정하고, 외부 자료 claim이 있으면 workflow state를 기록하고, staged gate를 통과할 때까지
수정합니다. 에이전트에게 이 시스템은 "어디를 읽고 어떤 순서로 움직일지"를 정하는 절차서입니다.

사람 관점에서 핵심은 **상태와 의미**입니다. 사람이 알고 싶은 것은 지금 어떤 결과가 남았고,
무엇이 아직 막혀 있으며, 다음에 어느 방향으로 갈 수 있는지입니다. 그래서 lineage 문서는
"어떻게 여기까지 왔나"를 설명하고, Source-X conclusions는 "무엇을 조사했고 어디서 막혔나"를
남기고, roadmap은 "현재 상태와 다음 gate가 무엇인가"를 말합니다. 사람에게 이 시스템은
채팅 로그가 아니라, 주문서와 진행표에 가깝습니다.

협업 관점에서 핵심은 **충돌과 책임 분리**입니다. 여러 에이전트가 같은 레포를 만질 때
각자 다른 상태표를 만들거나, 캐시하지 않은 자료를 "없다"고 말하거나, 약한 comparator를
증거처럼 쓰면 바로 충돌이 납니다. 그래서 팀 규칙은 status SSOT, workflow SSOT, staged
guard, pre-commit gate로 분리됩니다. 협업자에게 이 시스템은 누가 어떤 파일을 고쳤는지보다
"어떤 claim이 어떤 gate를 통과했는가"를 보게 하는 운영 장치입니다.

이 세 관점은 서로 다른 문서를 새로 만들라는 뜻이 아닙니다. 같은 파일을 읽되 질문이 다릅니다.
에이전트는 행동 규칙을 읽고, 사람은 진행 상태를 읽고, 팀은 재발 방지와 충돌 해결 규칙을
읽습니다.
...
문헌 후보 탐색
  ...
  -> [PageIndex](https://github.com/VectifyAI/PageIndex)·[MarkItDown](https://github.com/microsoft/markitdown) 파서 라우팅
...
### 4.2 [PageIndex](https://github.com/VectifyAI/PageIndex)와 [MarkItDown](https://github.com/microsoft/markitdown)의 역할 분리
...
| 입력 | 기본 경로 | 이유 |
| --- | --- | --- |
| 논문 PDF | [PageIndex](https://github.com/VectifyAI/PageIndex) | 페이지, 섹션, 수식, caption 구조가 중요합니다. |
| arXiv source / TeX | TeX 직접 확인 | Figure 번호와 원본 그림 파일 매핑의 ground truth입니다. |
| Zenodo README, JSON, release note | [MarkItDown](https://github.com/microsoft/markitdown) | 논문 본문이 아니라 매니페스트·표·코드 설명을 Markdown으로 바꾸는 데 적합합니다. |
| figure PDF / PNG | Poppler·ImageMagick | 수치화를 위해 고해상도 PNG를 렌더링해야 합니다. |

[PageIndex](https://github.com/VectifyAI/PageIndex)는 논문 PDF를 페이지와 섹션 단위로 읽기 위한 MCP 도구입니다. MCP는 AI가 외부
...
본문을 가져오라고 정합니다. [MarkItDown](https://github.com/microsoft/markitdown)은 데이터 릴리스, README, JSON, notebook처럼
논문 본문이 아닌 매니페스트성 자료를 Markdown 글로 바꾸는 데 더 적합했습니다.

여기서 MCP화는 물리 판단을 MCP에 맡긴다는 뜻이 아닙니다. [PageIndex](https://github.com/VectifyAI/PageIndex)는 MCP server로
등록해 `process_document`, `get_document_structure`, `get_page_content` 같은 호출을 에이전트가
쓸 수 있게 만든 경로입니다. [MarkItDown](https://github.com/microsoft/markitdown)은 이 문서의
운영 기록에서는 MCP 판단기가 아니라 로컬 `markitdown` 변환기로 호출한 non-paper asset route입니다.
둘 다 산출물 생성 도구일 뿐이고, 산출물의 연구 상태는 workflow state와 별도 검증 문서가 정합니다.

[회고 005: 연구 자산 디지털화 자동화](../postmortem/005-20260611-dorito-research-asset-digitization-automation.md)는
이 분리를 만든 사건 기록입니다. 그 회고는 [MarkItDown](https://github.com/microsoft/markitdown)으로 논문 PDF 전문을 처리하면
2단 레이아웃, 수식, figure reference, caption 순서가 깨질 수 있다고 정리합니다.
반대로 [PageIndex](https://github.com/VectifyAI/PageIndex)는 페이지·섹션 구조와 본문 chunk를 안정적으로 가져오는 데 유리했습니다.
다만 [PageIndex](https://github.com/VectifyAI/PageIndex)도 figure 번호와 원본 파일명을 확정하는 최종 기준은 아니었습니다. 회고 005는
[PageIndex](https://github.com/VectifyAI/PageIndex) caption 순서로 Amendola/Farrah figure mapping을 추정했다가 틀렸고,
arXiv TeX의 `\includegraphics`와 `\caption` block으로 교정했다고 기록합니다. Chen 논문처럼
TeX가 출판본보다 figure block을 더 많이 가진 draft superset일 때는 compiled PDF와 [PageIndex](https://github.com/VectifyAI/PageIndex)
page context로 다시 대조해야 했습니다.

Figure 매핑도 별도 무결성 문제였습니다. [PageIndex](https://github.com/VectifyAI/PageIndex) caption 순서만 믿으면 실제 Figure 번호와
...
여기서 중요한 점은 state token이 단순 장식이 아니라는 것입니다. state token은 claim의
강도를 낮추고, 다음 세션이 같은 자료를 어디서 이어받아야 하는지 알려 주는 최소 상태표입니다.

예를 들어 다음 두 문장은 다릅니다.

```text
NASA/LAMBDA 자료를 확인했다.
NASA/LAMBDA page-family asset은 asset_cached 상태다.
```

첫 문장은 사람이 보기에는 그럴듯하지만, 다음 에이전트에게는 아무 것도 확정해 주지 못합니다.
검색했는지, URL만 열었는지, HTML을 저장했는지, figure asset을 받았는지, 수치 제품을 만들었는지
알 수 없기 때문입니다. 두 번째 문장은 workflow state ladder의 특정 칸을 가리킵니다. 그래서
다음 에이전트는 "이미 cached 상태인 asset에서 extraction을 이어가면 된다" 또는 "cached일 뿐
digitized는 아니다"처럼 이어받을 수 있습니다.

agentic loop에서는 이 상태가 실제 실행 순서를 바꿉니다.

```text
문서 작성
  -> git add
  -> agent_workflow_guard.py --staged 실패
  -> 누락된 workflow marker 또는 state token 추가
  -> 다시 git add
  -> staged gate 재실행
...
따라서 "workflow state token을 추가했습니다. 다시 stage하고 gate를 재실행합니다"라는 말은
문장 수정을 했다는 뜻만이 아닙니다. 에이전트가 하드 게이트의 실패를 받아들이고, claim의
상태를 명시한 뒤, 같은 커밋 후보를 다시 검증 루프에 넣었다는 뜻입니다. 이 절차 때문에
soft reminder를 놓쳐도 commit gate가 다시 잡습니다.

세 관점으로 보면 의미가 더 분명합니다.

| 관점 | workflow state token의 의미 |
| --- | --- |
| 에이전트 | 다음에 어떤 workflow 단계부터 이어갈지 알려 주는 실행 상태입니다. |
| 사람 | 이 문장이 자료 존재, 캐시, 추출, 수치화, 부재 중 어디까지 말하는지 보여 주는 독해 표지입니다. |
| 협업 | Codex, Claude Code, 사람 reviewer가 같은 claim 강도를 공유하게 하는 충돌 방지 장치입니다. |

이 때문에 state token 없이 "자료 없음", "캐시 완료", "제품 없음", "coverage 확인" 같은 문장을
쓰면 안 됩니다. 그런 문장은 사람이 보기에는 결과처럼 보이지만, 협업 시스템에서는 재현 가능한
상태가 아닙니다.
