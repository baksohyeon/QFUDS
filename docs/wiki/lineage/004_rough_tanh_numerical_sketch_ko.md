...
질문은 하나였다. **이 거친 모델을 실제로 끝까지 밀면 ΛCDM(표준 우주론 모형) 대비 어디까지 갈까?**
...
| 축 | 발견 | 판정 |
| --- | --- | --- |
| 배경팽창 | V1은 z=5에서 H(허블 팽창률)가 ΛCDM의 약 절반(E비 = H/H₀ = 0.51). V2는 z=5에서 1.03이고 거리지수 차이가 전 구간 0.018 mag 미만(Type Ia 초신성 산포 ≈0.05 mag 이내) | V1은 초기우주에서 즉시 깨짐. V2 배경은 초신성으로 ΛCDM과 구별 불가 |
| 구조성장(S8 = σ₈·√(Ω_m/0.3), 구조 진폭 지표) | V2가 성장을 약 19% 억제. σ₈ 0.81 → 0.66, S8 0.83 → 0.67 | 방향은 메모 주장대로 S8를 낮춤. 다만 관측이 원하는 ≈0.76보다 더 내려가 과도하게 오버슈트 |
| 위상(DESI(암흑에너지 분광 관측) 비교) | QFUDS tanh는 w가 0 → -1(시간이 갈수록 더 음으로, freezing = 동결 방향). DESI w0wa 중심값은 w가 약 -1.7 → -0.7(thawing = 해동 방향). z=2에서 각각 -0.53 vs -1.37 | 부호와 메커니즘이 정반대 영역(regime). 메모가 기대한 "DESI와 일치"는 성립하지 않음 |
...
z=2보다 더 일찍 두고 이동도(mobility)를 맞춰야 한다. tanh는 이 결합을 z=2에
손으로 박아 넣어 숨겼던 것이다. 부수적으로 lag와 hysteresis는 이완동역학의 비가역성, 즉 시간
...
줄었다. 전이 시점은 손으로 깔아 넣는 입력이 아니라 임계밀도에서 유도되고, 거기에 CP2의
지연이 얹힌다.
...
CP4가 가리킨 진짜 레버를 직접 돌렸다. 여기서 c_eff²(유효 음속의 제곱, 전이 성분이
얼마나 잘 뭉치느냐를 결정하는 손잡이)가 1에 가까우면 전이 성분이 매끄러워서 군집을
안 하므로 성장이 과하게 눌리고, c_eff²가 작으면 물질처럼 군집해서 안 눌린다. 그
사이 어딘가에서 관측 S8≈0.76이 나올 수 있을까?
...
즉 CP6a는 **050 천장을 넘지 못한다.** 오히려 천장을 *수치로* 확정했다: 단일 척도
가정이 자유 파라미터를 하나도 제거하지 못하고, c_eff²는 canonical(표준) 값보다
4.5자릿수 아래로 튜닝돼야 하며 그것을 메울 foam 메커니즘이 없다. 상전이 동역학은 형태만 줄 뿐,
...
애매한 손잡이(자유 파라미터)를 (문헌 기반 prior 범위에서) 격자로 훑어 데이터에
최대한 끼워 맞췄다. 스캔: z\*∈{2..6}, c_eff²∈logspace(-7,-2), Ω_m0∈{0.29,0.31,0.33}.
...
물리적 손잡이 = 질서변수(order parameter)의 **상관길이 ξ**(공간적으로 한 점의
...
새로 날카롭게 만든 구간이 정확히 그 z>2 — Lyman-α(수소 흡수선) 숲 BAO(바리온
...
아직 죽이긴 어렵지만 D_H/r_d가 가장 가까운 손잡이. 부호는 freezing으로 DESI의
...
구별하는 SNR ≈ **24σ** — 상수-γ MG는 진폭 주변화에 흡수되고, running은 그 잔차 스케일
의존성으로 살아남는다.
...
CP1–CP19가 남긴 빈자리 스펙시트는 {ξ≈10 Mpc, N_X≈2e5, z*≈2}였다. CP20은 이걸 foam
order-parameter ansatz에서 **유도 시도**한다 — 돌파를 노리는 게 아니라, "050
blocked"를 *이름 붙은 기존 물리 문제들로 분해*해서 진짜 유도가 뭘 내놔야 하는지
재확정한다. 이건 탐색적 시도지 유도가 아니다. **050 결론도 로드맵도 바꾸지 않는다.
observer mode 그대로**([`cp20_ceiling_derivation.py`](assets/004_rough_tanh/cp20_ceiling_derivation.py),
[`fig_cp20_ceiling_derivation.png`](assets/004_rough_tanh/fig_cp20_ceiling_derivation.png)).

가장 관대한 honest ansatz: 암흑섹터를 단일 order parameter φ + Ginzburg–Landau
자유에너지 F=∫[½K(∇φ)²+V(φ)]로. 두 표준 사실: (1) 상관길이 ξ=√(K/|V''|), 섭동
음속도 같은 stiffness가 정함 → c_s≈ξ/d_H (CP8). (2) 전이는 물질밀도가 임계 ρ*를
지날 때 (CP3). 그러면 "자유 손잡이" 3개는 사실 미시 입력 {K, V'', ρ*}다. foam이
이걸 고정하나?
...
빛으로도 신호 교환을 못한 거리에서 결함이 얼어붙음)** — ξ ≈ 전이 시점의 인과
지평선이다.
그걸 넣으면 그림이 양쪽이 된다.
...
→ **10 Mpc는 양쪽 horn 어디에도 자연스럽지 않다.** 미시 foam엔 너무 크고 인과
지평선엔 너무 작다. 맞추려면 tuned 근임계(ξ∝|T−Tc|^−ν)나 tuned quench rate가
필요하다 — 튜닝을 위치만 옮긴다. 게다가 데이터 ξ는 **비선형/σ8 스케일**(표준 물리
k_eq·transfer·growth가 정함)이라, 그걸 재현하는 건 *fit이지 새 정보가 아니다*(독립
증거력 0).

**Target 2 (N_X):** GL ansatz 안에선 c_s²=(ξH0/c)²라 N_X=1/c_s²−1이 **ξ 함수** →
"둘"이 실은 하나. 단 이 항등식은 d_H를 끌어들인 proxy라, 일반 P(X,φ)면 분리될 수
있다 — 그래도 *underived 숫자 2개로 되돌아갈 뿐* 도움 안 됨.

**Target 3 (z*=ρ*):** 관측 가능한 전이(0.1<z*<10)이려면 ρ*가 ρ_Λ 둘레 **≈3자릿수
밴드**(전체 ≈120자릿수 중)에 앉아야 한다 = **cosmic coincidence(why-now)**.
tracker/attractor quintessence는 초기조건 튜닝은 풀지만 튜닝을 **meV 퍼텐셜
스케일로 옮길** 뿐, why-now는 안 푼다.

**판정: 스펙 3개 → 고전적 미해결 문제 2개(스케일 + 코인시던스)로 분해된다.** foam은
둘을 *이름만 바꿀 뿐 풀지 않는다*. 050 천장 reconfirmed, 단 *왜 막히나*가
날카로워졌다: (1) 10 Mpc는 미시 foam에도 인과 지평선에도 안 맞아 tuned 근임계를
요구하고, (2) ρ*≈ρ_Λ는 why-now. 진짜 유도가 내놔야 할 것: **유도된 Tc/quench로
emergent하게 tuned-down된 ξ + ρ*↔ρ_Λ를 동역학적으로 묶는 메커니즘.** 둘 다 없다.
진짜 검증은 CLASS/hi_class(Level 3, blocked).
...
## CP21 (2026-06-12): ξ≈9.5 Mpc를 근임계·Kibble-Zurek로 brute-force 하면 손잡이는 줄지 않고 옮겨갈 뿐

CP20은 데이터가 원하는 상관길이 ξ≈9.5 Mpc가 미시 foam 척도(10⁴–10⁵⁸배 너무
작음)에도, z≈2의 인과/Kibble 지평(≈4400 Mpc, 461배 너무 큼)에도 자연스럽지 않다는
것을 확정했다. 유일한 탈출구로 CP20이 지목한 것은 **창발적으로(emergent) 줄어든 ξ** —
근임계성(ξ∝|T−Tc|^−ν)이나 Kibble-Zurek 동결이다. CP21은 그 두 메커니즘을 9.5 Mpc에
강제로 맞춰 본다. 핵심은 "맞췄다"가 아니다(brute-force는 항상 맞출 수 있다). 핵심은
**튜닝 장부**다: 맞추는 과정에서 fine-tune할 숫자의 *개수*가 주는가(유도로의 진전),
아니면 그 튜닝이 메커니즘 자신의 손잡이로 *옮겨갈* 뿐인가(진전
없음)?([`cp21_xi_criticality.py`](assets/004_rough_tanh/cp21_xi_criticality.py),
[`fig_cp21_xi_criticality.png`](assets/004_rough_tanh/fig_cp21_xi_criticality.png))

근임계성은 ε=(ξ0/ξ_target)^(1/ν)을 풀면 끝난다. Planck 척도 ξ0에서 출발하려면 계는
임계점에서 ε≈10⁻⁵⁹(ν=1)–10⁻¹¹⁷(ν=0.5)만큼 떨어진 곳에 앉아 있어야 한다 — 말도 안
되는 칼날 위 근접도다. 응답 민감도 d ln ξ/d ln ε=−ν는 O(1)로 완만하지만, 튜닝은
*기울기*가 아니라 ε의 *작음(값) 자체*에 들어 있다. Kibble-Zurek은
τ_Q/τ0=(ξ_target/ξ0)^((1+νz)/ν)을 풀면 되는데, Planck ξ0에서 우주적 quench가 미시
시간보다 10¹¹⁶(z=1)–10²³³(z=3)배 느려야 한다. 지수 σ=ν/(1+νz)∈(0.2,0.5)이
sub-linear라 *응답*은 근임계성보다 부드럽지만, 요구되는 중심값 자체는 여전히
외부에서 공급해야 하는 거대한 input이지 output이 아니다.
...
```text
- 근임계성: 9.5 Mpc를 맞추려면 ε를 10⁻⁵⁹–10⁻¹¹⁷(Planck ξ0)까지 칼날같이 fine-tune해야 함.
- Kibble-Zurek: τ_Q/τ0를 10¹¹⁶–10²³³까지 키워야 함. σ는 sub-linear라 민감도만 완만(0.2–0.5).
- 두 경우 모두 손잡이 개수 reduced 아니라 relocated — ξ의 튜닝이 ε/τ_Q로 옮겨가고 ν·z·ξ0가 덧붙음.
...
- LTB(Lemaître–Tolman–Bondi, 구면 대칭 비균질 우주)와 void model은 ΛCDM 대안으로
  ...
  거대 보이드 한계)와 CMB 등방성으로 ~1% 수준에서 배제**됨 (Caldwell & Stebbins
  arXiv:0711.3003).
...
- chameleon screening(Khoury & Weltman arXiv:astro-ph/0405033), symmetron, dilaton —
...
**결정적 한 방**: scope mismatch. screening은 sub-Mpc 국소 효과, z* 시프트는 ~Gpc
...
**결정적 한 방**: 8% H0 갭을 메우려면 Q_D가 ~O(10%)여야 하는데 학계 합의는
~O(0.1–1%)다. **두 자릿수 이상 모자라다.** 더해 CP10의 결론(S8와 H0 분리)은 평균화
오류가 *아니라* 모델 *설계*(섭동 손잡이 c_eff² ≠ 배경 손잡이)에서 나온 것이라
...
스케일(~10 Mpc)을 cutoff* 로 본다면 어떻게 되나? 발육기(태아·유아·사춘기) 우주의
...
| L 선택 | 학명 | 핵심 인용 |
| --- | --- | --- |
| Hubble 지평 (~c/H₀ ≈ 4400 Mpc) | 원조 HDE | Cohen–Kaplan–Nelson 1999; Li 2004 |
...
| 비선형/구조 스케일 (~Mpc) | EFT of LSS (cutoff) | Baumann et al. 2012; Carrasco–Hertzberg–Senatore 2012 |

원조 HDE는 정확히 사용자의 "미시·양자에 갇혀서 헤맸다"는 비판을 1999년에 형식화한
것이고, EFT of LSS는 *비선형 스케일 ~10 Mpc*을 dark sector의 coarse-graining
...
지평·미래·Ricci·Barrow·Tsallis 모두 L *선택*이고, 그 선택은 다시 손잡이로 남는다.
CP23의 "transmutation/anthropic/SUSY/sequestering 모두 relocate일 뿐" 진단과 정확히
같은 모양 — HDE도 "Planck UV cutoff 미스터리"를 "왜 IR cutoff가 이 스케일인가
미스터리"로 *옮길* 뿐이다.
