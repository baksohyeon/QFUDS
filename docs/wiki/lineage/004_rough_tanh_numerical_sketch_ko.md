...
새로 날카롭게 만든 구간이 정확히 그 z>2 — Lyman-α 숲 BAO다. 그래서 이번엔 CPL
...
못박는다. 이건 탐색적 시도지 유도가 아니다. **050 결론도 로드맵도 안 바꾼다.
...
메커니즘은 미시가 아니라 **인과적(Kibble)** — ξ ~ 전이 시점의 인과 지평선이다.
그걸 넣으면 그림이 양쪽이 된다.
...
ξ: 양쪽 다 안 맞음 — 미시 foam 10^4~10^58배 작고, 인과/Kibble 지평선 461배 큼.
...
것을 못박았다. 유일한 탈출구로 CP20이 지목한 것은 **창발적으로 줄어든 ξ** —
...
```text
- 근임계성: 9.5 Mpc를 맞추려면 ε를 10^−59~10^−117(Planck ξ0)까지 칼날같이 fine-tune해야 함.
- Kibble-Zurek: τ_Q/τ0를 10^116~10^233까지 키워야 함. σ는 sub-linear라 민감도만 완만(0.2~0.5).
