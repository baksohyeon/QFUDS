...
둘의 결합을 z=2에 손으로 끼워 넣어 숨겼던 것이다. 덧붙여, 지연(lag)과
...
움직이고, **관측 S8=0.76은 단일 S8 교차 기준 c_eff² ≈ 2.92e-5에서 달성된다.**

![음속 c_eff²를 작게 만들수록 암흑성분이 더 잘 뭉쳐 S8이 올라가며, 관측치 0.76은 c_eff²≈2.92e-5에서 정확히 지난다](assets/004_rough_tanh/fig_cp5_sound_speed.png)

| c_eff² | S8 |
| --- | --- |
| 1.0 (매끄러움) | 0.677 |
| 1e-4 | 0.726 |
| **≈2.92e-5 (single-S8 fit)** | **0.760** |
| 1e-6 | 0.887 |
| 1e-8 (완전 군집) | 0.955 |

즉 CP4의 오버슈트는 **치명적이 아니라 고칠 수 있다.** 단, 그 대가는 **아주 작은
음속(c_eff²≈2.92e-5)이라는 파라미터를 하나 더 손으로 맞추는 것**이다. 이것이 정확히
003이 말한 "c_eff²=0은 해결책이 아니라 미해결 조건"이다. 이제 그 미해결 조건에
숫자가 붙었다: 관측을 맞추려면 c_eff²가 1보다 5자리수 작아야 한다.

주의: 이 η는 거친 프록시다. 정확한 값(2.92e-5인지 1e-4인지)은 CLASS의 clustering-DE
...
```text
S8 오버슈트는 치명적이 아니다. c_eff² ≈ 2.92e-5에서 관측 0.76이 나온다.
...
CP5까지 오면서 모델이 한 관측을 통과할 때마다 직접 정해야 하는 값(전이 시점 z*, 전환
폭 Δa, 골 사이 장벽, 이동도 M, 음속 c_eff²)이 늘었다. 그래서 두 가지를 동시에 물었다.
이 자유도들이 거품의 단일 척도에서 함께 따라 나올 수 있는가(6a), 아니라면 이 모델은
어느 관측에서 가장 먼저 틀렸다고 판정할 수 있는가(6b)?

쉽게 말하면 이렇다. 지금까지는 관측 하나를 맞출 때마다 별도 조절값이 하나씩 늘어난
셈이다. 6a는 그 조절값들이 하나의 물리 축으로 묶이는지 확인하고, 6b는 모델이 깨진다면
어느 관측이 가장 먼저 신호를 줄지 찾는다.
...
필요하다. 그런데 CP5가 S8를 맞추기 위해 요구한 값은 c_eff²≈2.92e-5였다.

비유하면, 어떤 재료 속에서 잔물결이 퍼지는 속도는 재료의 성질이 정하지 우리가 마음대로
정하지 못한다. 표준 스칼라장이라는 '재료'는 그 속도(c_eff²)를 1로 고정하는데, 데이터는
그보다 4.5자릿수나 느린 값을 요구한다. 그 차이를 메워 줄 재료가 거품 쪽에 따로 없다는
게 핵심이다.

- 자연스러운 canonical c_eff² = 1 -> S8 = 0.677 (오버슈트)
- S8=0.76 요구 c_eff² ≈ 2.92e-5 -> 둘 사이 **약 4.5 자릿수 갭**
...
## CP7 (2026-06-12): brute-force 현상론 피팅 - 적합 한계 확인

CP6b는 이 모델이 배경 관측으로는 ΛCDM과 구별되지 않고, 가장 취약한 지점이 DESI의
w(z)임을 보여 줬다. 그렇다면 반대로 물어볼 수 있다. 허용된 사전 범위 안에서 데이터를
최대한 맞추면, 명목 점수상 ΛCDM을 이기는 지점까지 갈 수 있을까? 이 질문을 직접
실행했다.
...
CP5–CP7은 c_eff²를 데이터에 맞게 돌렸다(CP5의 단일 S8 교차값은 2.92e-5,
CP7의 거친 격자 best는 4.6e-6). CP8은 묻는다. **어떤 물리량이 그 값을
*내놓나*, 아니면 거품 전제는 다른 값을 주나?**
...
쉽게 풀면 이렇다. CMB에는 초기 우주가 남긴 '표준 자'(소리 파동이 굳어 생긴 길이 r_s,
재결합 = 우주가 식어 빛이 자유로워진 시점 이전 물리가 정함)가 하나 있다. QFUDS의 전환은
한참 뒤(z≈2)라 그 이전은 ΛCDM과 똑같으니, 자의 실제 길이도 안 바뀌고 하늘에 보이는 그
자의 각도 θ*도 고정이다. 그러면 남는 질문은 "그 자까지 거리를 맞추려면 지금 팽창률
H0가 얼마여야 하나"뿐이라, H0가 후기 팽창사 ∫dz/E(z) 하나로 결정된다. 가까운 별부터
차근차근 거리를 재 올라가는 보통의 '거리 사다리'와 방향이 반대(먼 CMB에서 거꾸로 내려옴)
라 inverse distance ladder(역거리 사다리)라고 부른다.
...
쉽게 말하면, 우주를 '보통 물질'과 '암흑 유체' 두 액체가 섞인 것으로 보고, 각 액체가
어디에 얼마나 뭉쳤는지(δ)와 어느 쪽으로 흐르는지(ϑ)를 시간에 따라 같이 푸는 것이다.
CP5·CP9에서 쓴 η는 이 얽힌 흐름을 '얼마나 뭉치나' 한 숫자로 줄인 어림이었고, 여기서는
그 어림 대신 4개의 변수를 직접 계산해 어림이 얼마나 빗나갔는지 잰다.
...
중력 포텐셜이 시간에 따라 변하면서 CMB 빛의 에너지를 조금 바꾸는 흔적)을 한 번도 안
건드렸다. ISW는 ΔT/T = 2∫(∂Φ/∂η)dη, 곧 **중력 포텐셜이 시간에 따라 변해야(Φ̇≠0)**
생긴다. 순수 물질우주는 D∝a라 Φ가 거의 일정하고 ISW=0; 가속하거나 성장이 눌리면 Φ가
시간에 따라 바뀌면서 ISW가 생긴다.

비유하면, 빛이 무거운 구조가 만든 중력 우물을 지나간다고 볼 수 있다. 들어갈 때와 나올
때 우물 깊이가 그대로면 들어가며 얻은 에너지를 나올 때 돌려주기 때문에 흔적이 안 남는다.
하지만 우주 팽창과 구조 성장 변화 때문에 빛이 지나가는 동안 우물의 깊이가 달라지면,
얻은 에너지와 돌려주는 에너지가 살짝 어긋난다. 그 작은 차이가 CMB에 온도 얼룩으로
남는 것이 ISW다. 그래서 ISW source를
...
w(z)(relax z*=5, barrier=2, mobility=2, lam=3)를 CPL (w0,wa)(w(a)를 현재값 w0와 변화율 wa 두 수로 요약하는 표준 매개화) 한 점으로 투영했다.
...
(σ는 차이의 크기를 '우연으로 이만큼 벌어질 확률'로 환산한 단위다. 숫자가 클수록 우연으로
보기 어렵다는 뜻이고, 5σ면 사실상 우연이 아니라고 보는 선이라 입자물리에서 발견 기준으로
쓴다. 즉 5.5σ는 "예시 DESI가 맞다면 이 방향 차이는 우연이 아니다"에 해당한다.)
...
k_J≈0.105 부근에서 켜지는 반면, 이 점성 proxy의 δ_X 비율은 스캔 최저 k부터 이미
강하게 눌려 있고 k≈1.2에서 0.95 수준으로 되돌아온다. 즉 “고-k onset”이 아니라
전혀 다른 ratio shape이다. 두 파라미터의 지문이 k에서 갈라진다.
...
k-모양도 다름: c_s² Jeans는 k_J≈0.1 부근에서 켜지고, 점성 ratio는 저-k부터 눌린 뒤 k≈1.2에서 회복.
...
CP15까지 이 모델의 자유도(음속, 점성)가 성장을 어떻게 누르는지 봤다. 여기서 성장은
우주의 크기가 커지는 팽창이 아니라, 밀도 요동이 커져 은하·구조가 더 뭉치는 정도다.
그렇다면 바깥에서 보는 사람은 이 눌린 성장을 보고 "표준 중력(ΛCDM)이네"라 할까, 아니면 "중력이
수정됐네"라 할까? 그걸 가리는 표준 진단 도구가 성장지수 γ다. CP16은 우리 모델을 그
진단으로 확인한다.
...
0.55에서 벗어남이 곧 "지문"이 된다. 먼저 ΛCDM 솔버가
...
쉽게 말하면, 두 렌즈는 서로 다른 깊이를 본다. 약중력렌즈는 비교적 가까운(z≈0.3–0.8)
구조를, CMB렌즈는 훨씬 먼(주로 z>1) 구조를 평균해서 본다. QFUDS의 전이는 가까운 쪽
(z≲1)에서만 성장을 바꾸므로, 먼 곳을 보는 CMB렌즈에는 거의 보이지 않는다. 그래서 둘이
서로 다른 답을 원해도 모순까지는 가지 않는다. 다시 말하면, 가까운 구조를 보는
약중력렌즈에는 효과가 보일 수 있지만, 먼 구조를 평균하는 CMB렌즈에는 그 효과가 약하다.
...
Euclid/DESI 후속 우주론 자료는 이 rough-tanh 가지의 다음 외부 검산 지점이다. 우리의 두
반증 후크는 모두 약한 중력렌즈가 들이댈 수 있는 도구다: CP9의 스케일 의존
...
미시물리에서 **유도한 것이 아니다**(hit ≠ derivation). 050 ceiling과 observer mode는
...
backreaction에 따른 H0 자기정합성), 수치 spike 이전에 학계 문헌부터 스캔했다.**
