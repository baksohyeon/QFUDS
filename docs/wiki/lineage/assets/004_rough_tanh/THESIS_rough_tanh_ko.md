...
![배경 팽창: V1은 초기우주에서 깨지고 V2는 ΛCDM과 거리지수 차이 <0.02 mag](fig_background.png)
...
![성장/S8: 전이 모양을 바꿔도 S8 오버슈트는 그대로 = 구조적](fig_cp4_growth.png)
...
![c_eff² 스캔: S8=0.76은 c_eff²≈3×10⁻⁵에서 달성](fig_cp5_sound_speed.png)
...
![c_eff² 도출: fit값 4.6×10⁻⁶ ↔ 상관길이 ξ≈10 Mpc = 거대구조 스케일](fig_cp8_ceff2_derivation.png)
...
![H0 테스트: inverse distance ladder로 66.18, −1.81% (악화)](fig_cp10_h0_test.png)
...
![렌즈 P(k): k_J≈0.1에서 스케일 의존 계단 vs 균일 저-σ8은 평평](fig_cp9_lensing_pk.png)
...
![ISW: C_ℓ 비가 0.35→0.82로 기울어짐 vs 균일 저-σ8은 flat 0.857](fig_cp13_isw.png)
...
![γ 지문: 단일 스케일 f(R)와 축퇴, 그러나 γ_eff(k) running으로 구별](fig_cp16_growth_index.png)
...
![kill test: w(z) freezing은 대표 DESI에서 반대 사분면, P(k) 스텝은 Stage-IV 사정권](fig_cp14_kill_test.png)
...
![CMB렌즈 내부정합: 늦은 유체라 z>1 가중 CMB렌즈를 거의 안 건드림](fig_cp17_cmb_lensing.png)
...
![DESI z>2 Lyα: D_H/r_d는 대표 정밀도 경계선, 가장 가까운 손잡이](fig_cp18_desi_highz_bao.png)
...
![Euclid forecast: P(k) 스텝 다중-bin ~24σ, γ running ~24σ로 구별](fig_cp19_euclid_forecast.png)
...
![유체/EFT 위치: GDM(c_vis²=0) 특수경우 + 강하게 비표준인 k-essence](fig_cp12_fluid_frameworks.png)
...
![050 천장 직격: 스펙 3개 → 스케일 문제 + 코인시던스 문제](fig_cp20_ceiling_derivation.png)
...
![ξ 근임계/Kibble-Zurek: 칼날 튜닝, relocated not reduced](fig_cp21_xi_criticality.png)
...
![coincidence tracker: IC 튜닝은 16 decade 진짜 제거, meV는 잔존](fig_cp22_coincidence_tracker.png)
...
![meV = 우주상수 문제: anthropic은 select, transmutation은 작음만 natural](fig_cp23_cc_problem.png)
...
![ξ = σ8 스케일: circular, 차원전환은 relocate, 독립 유도 후보 없음](fig_cp24_scale_problem.png)
...
실행 목록은 `004_rough_tanh_numerical_sketch_ko.md`의 '재현' 절을 참조.
