...
> 못했다고 닫는다.
> NASA+BAO baseline map도 이에 맞춰 관측 kill-map 전용으로 재정렬됐으며,
> model-facing 해석은 여전히 금지된다.
...
채우지 못한다고 기록했다.
이에 따라 NASA+BAO baseline map은 관측 kill-map 전용으로 재정렬됐고, `xi`, transition width,
transition redshift, amplitude를 NASA/BAO에서 고른 뒤 foam source로 부르는 경로는 계속 금지된다.
