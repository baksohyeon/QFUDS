...
따라 흐르는 성장지수 γ_eff(k). 대표 Euclid급 토모그래픽 예측에서 이 계단은 약 24σ로
...
방법론적으로, 본 연구는 인식론적 규율을 강제하는 **자기교정 AI 에이전트 하네스**
위에서 24개 원자적 체크포인트로 수행됐다. 이 하네스는 (i) 운영 워크플로 SSOT
...
### 2.4 연구 인프라: 자기교정 AI 에이전트 하네스 (Self-correcting agent harness)

본 연구의 24개 체크포인트는 단일 연구자가 손으로 돌린 것이 아니라, **인식론적 규율을
강제하는 AI 에이전트 하네스(harness)** 위에서 수행됐다. 투기적 가설을 끝까지 밀면서도
...
요컨대 이 하네스는 **"가짜 돌파가 조용히 살아남을 수 없도록"** 설계됐다. 표준
...
이 인프라 덕분이다. *자기교정이 내장된 AI 연구 하네스가 과대주장 없는 투기적 탐색을
...
차이가 전 구간 |Δμ| < 0.02 mag로 **초신성 산포(약 0.05 mag) 안에 숨는다.** 밀도구동
...
V2는 구조성장을 약 19% 억제해 S8를 0.83→0.67로 *과하게* 낮춘다(관측값 약 0.76 아래로
...
초신성에 고정돼(§3.1) 움직일 여지가 없다. H0를 풀려면 재결합 이전(r_s)을 바꿔야
...
뒤에도 대표 Stage-IV 설정에서 약 18σ의 모양 신호로 남고, 균일 이동은 0σ(sanity).
...
![Euclid forecast: P(k) 스텝 다중-bin 약 24σ, γ running 약 24σ로 구별](assets/004_rough_tanh/fig_cp19_euclid_forecast.png)

대표 Euclid급 토모그래픽 약중력렌즈 Fisher 예측에서, 진폭을 bin별로 주변화한 뒤에도
P(k) 계단은 **약 24σ**, γ_eff(k) running은 상수-γ 수정중력과 **약 24σ**로 구별된다.
...
- **스케일 문제**: ξ≈10 Mpc은 미시 거품(너무 작음)에도, z≈2 인과/Kibble
  지평(약 4400 Mpc 공변, 461배 큼)에도 자연스럽지 않다. 그것이 일치하는 유일한
  스케일은 비선형/σ8 구조 스케일(R8≈12 Mpc)인데, 이는 표준 물리가 정하는 값이라
  재현해도 독립 증거력이 0이다(매칭≠유도). N_X는 ξ로 환원되어 둘은 사실 하나다.
- **코인시던스 문제**: 전이가 관측 가능하려면 임계밀도 ρ*가 ρ_Λ의 약 3 order 안
  (전체 약 120 order 중)에 앉아야 한다. 바로 why-now 문제 그 자체다.
...
(약 95 Mpc), r_d(약 147 Mpc), z≈2 지평(약 4400 Mpc)은 전부 크기가 맞지 않는다. 차원전환은
튜닝을 결합상수로 relocate할 뿐(10⁵⁸ 길이 튜닝을 약 0.26% 결합 튜닝으로 줄이는 부분
...
3. **세 반증 신호를 남긴다**: P(k) 스케일 의존 계단(k≈0.1), ISW 스케일 의존 기울기,
   γ_eff(k) running. Euclid급 토모그래피에서 약 24σ로 검출 가능.
...
결합한 자기교정 AI 연구 하네스가, 투기적 가설을 끝까지 밀면서도 가짜 돌파를 구조적
...
| CP14 | 실오차 채점 | freezing 반대 사분면, P(k) 약 18σ | `fig_cp14_kill_test` |
...
| CP19 | Euclid forecast | P(k) 약 24σ, γ running 약 24σ | `fig_cp19_euclid_forecast` |
