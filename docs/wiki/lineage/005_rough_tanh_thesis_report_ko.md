...
구별 불가능하게 재현하고, 작은 유효 음속
($c_{\mathrm{eff}}^2 \approx 10^{-5}$)을 도입하면 $S_8$를 관측값 쪽으로 낮출 수 있다.
...
$c_{\mathrm{eff}}^2 \approx 3\times10^{-5}$에서 나온다.
...
$c_{\mathrm{eff}}^2 \approx 3\times10^{-5}$에서 달성된다.** 즉 오버슈트는 즉시 배제
...
다음 단계는 둘로 제한된다. 하나는 2026년 Euclid 우주론 자료와 DESI 후속으로 §4의 반증
신호를 검증하는 일이다. 다른 하나는 $\xi \approx 10\,\mathrm{Mpc}$와
...
- **CP5 유효 음속 스캔.** $S_8=0.76$은
  $c_{\mathrm{eff}}^2 \approx 3\times10^{-5}$에서 맞지만, 이는 튜닝이다.
