---
doc_id: wiki_governance_002_terminology_backlink_check
title: Terminology Backlink Consistency Check
doc_type: summary
stage: "2"
status: completed
evidence_role: audit
depends_on:
  - repository_levels_glossary
next_gate: maintain first substantive level-term backlink when editing affected docs
last_updated: 2026-06-11
---

# Terminology Backlink Consistency Check

## Scope

This audit implemented backlinks from maintained documents that mention
[repository levels](../glossary/repository_levels.md). It did not rewrite level
definitions, duplicate glossary content, or change roadmap status.

## Files Updated

- [PROJECT.md](../../../PROJECT.md)
- [README.md](../../../README.md)
- [docs/00_project/concept_survival_audit.md](../../00_project/concept_survival_audit.md)
- [docs/00_project/decision_log.md](../../00_project/decision_log.md)
- [docs/00_project/overview.md](../../00_project/overview.md)
- [docs/00_project/project_identity.md](../../00_project/project_identity.md)
- [docs/00_project/qfuds_ko.md](../../00_project/qfuds_ko.md)
- [docs/00_project/qfuds_positioning.md](../../00_project/qfuds_positioning.md)
- [docs/00_project/research_program.md](../../00_project/research_program.md)
- [docs/00_project/success_criteria.md](../../00_project/success_criteria.md)
- [docs/00_project/traceability_matrix.md](../../00_project/traceability_matrix.md)
- [docs/00_project/verification_guide.md](../../00_project/verification_guide.md)
- [docs/02_theory/015_qfuds_level_1_5_equivalence_source_perturbation_audit.md](../../02_theory/015_qfuds_level_1_5_equivalence_source_perturbation_audit.md)
- [docs/02_theory/015_qfuds_level_1_5_transfer_four_vector_derivation_attempt.md](../../02_theory/015_qfuds_level_1_5_transfer_four_vector_derivation_attempt.md)
- [docs/02_theory/015_qfuds_qnu_necessity_formulation_audit.md](../../02_theory/015_qfuds_qnu_necessity_formulation_audit.md)
- [docs/02_theory/015_qfuds_strong_gravity_source_mechanism_audit.md](../../02_theory/015_qfuds_strong_gravity_source_mechanism_audit.md)
- [docs/02_theory/015_qfuds_v0_15_phase_transfer_physics.md](../../02_theory/015_qfuds_v0_15_phase_transfer_physics.md)
- [docs/02_theory/030_qfuds_phenomenological_perturbations.md](../../02_theory/030_qfuds_phenomenological_perturbations.md)
- [docs/02_theory/README.md](../../02_theory/README.md)
- [docs/03_experiments/010_exp_001_gamma_scan.md](../../03_experiments/010_exp_001_gamma_scan.md)
- [docs/03_experiments/015_exp_001_5_phase_transfer_physicality.md](../../03_experiments/015_exp_001_5_phase_transfer_physicality.md)
- [docs/03_experiments/020_exp_002_entropy_information_gate.md](../../03_experiments/020_exp_002_entropy_information_gate.md)
- [docs/03_experiments/030_exp_003_phenomenological_perturbation_closure.md](../../03_experiments/030_exp_003_phenomenological_perturbation_closure.md)
- [docs/03_experiments/030_exp_004_p1_model_family_positioning.md](../../03_experiments/030_exp_004_p1_model_family_positioning.md)
- [docs/03_experiments/030_exp_005_timing_prior_usefulness.md](../../03_experiments/030_exp_005_timing_prior_usefulness.md)
- [docs/03_experiments/030_exp_006_literature_timing_support_audit.md](../../03_experiments/030_exp_006_literature_timing_support_audit.md)
- [docs/03_experiments/README.md](../../03_experiments/README.md)
- [docs/04_results/000_experiment_summary.md](../../04_results/000_experiment_summary.md)
- [docs/04_results/010_result_001_gamma_scan.md](../../04_results/010_result_001_gamma_scan.md)
- [docs/04_results/015_result_001_5_phase_transfer_physicality.md](../../04_results/015_result_001_5_phase_transfer_physicality.md)
- [docs/04_results/020_result_002_entropy_information_gate.md](../../04_results/020_result_002_entropy_information_gate.md)
- [docs/04_results/030_result_003_phenomenological_perturbation_closure.md](../../04_results/030_result_003_phenomenological_perturbation_closure.md)
- [docs/04_results/030_result_004_p1_model_family_positioning.md](../../04_results/030_result_004_p1_model_family_positioning.md)
- [docs/04_results/030_result_005_timing_prior_usefulness.md](../../04_results/030_result_005_timing_prior_usefulness.md)
- [docs/04_results/030_result_006_literature_timing_support_audit.md](../../04_results/030_result_006_literature_timing_support_audit.md)
- [docs/04_results/README.md](../../04_results/README.md)
- [docs/05_next_steps/000_roadmap.md](../../05_next_steps/000_roadmap.md)
- [docs/05_next_steps/010_perturbation_gate.md](../../05_next_steps/010_perturbation_gate.md)
- [docs/05_next_steps/010_safe_future_branch_candidates.md](../../05_next_steps/010_safe_future_branch_candidates.md)
- [docs/05_next_steps/015_level_1_5_resolution_gate.md](../../05_next_steps/015_level_1_5_resolution_gate.md)
- [docs/05_next_steps/015_retained_branch_source_closure_plan_provenance.md](../../05_next_steps/015_retained_branch_source_closure_plan_provenance.md)
- [docs/05_next_steps/030_exp003_record_consistency_gate.md](../../05_next_steps/030_exp003_record_consistency_gate.md)
- [docs/05_next_steps/900_roadmap_overview_ko.md](../../05_next_steps/900_roadmap_overview_ko.md)
- [docs/05_next_steps/README.md](../../05_next_steps/README.md)
- [docs/README.md](../../README.md)
- [docs/wiki/governance/001_physical_branch_admission_summary.md](001_physical_branch_admission_summary.md)
- [docs/wiki/governance/003_blocked_admission_rule_gate.md](003_blocked_admission_rule_gate.md)
- [docs/wiki/governance/004_missing_physics_map.md](004_missing_physics_map.md)
- [docs/wiki/lineage/001_qfuds_genealogy.md](../lineage/001_qfuds_genealogy.md)
- [docs/wiki/lineage/002_branch_graph.md](../lineage/002_branch_graph.md)
- [docs/wiki/postmortem/001-20260609-dorito-li-2025-data-cache.md](../postmortem/001-20260609-dorito-li-2025-data-cache.md)
- [docs/wiki/postmortem/002-20260609-dorito-iv-ide-timing-checkpoint.md](../postmortem/002-20260609-dorito-iv-ide-timing-checkpoint.md)
- [docs/wiki/postmortem/003-20260610-dorito-qfuds-scope-demotion-retrospective.md](../postmortem/003-20260610-dorito-qfuds-scope-demotion-retrospective.md)
- [docs/wiki/research/investigations/exp006_timing/008_iv_ide_timing_synthesis_ko.md](../research/investigations/exp006_timing/008_iv_ide_timing_synthesis_ko.md)
- [docs/wiki/research/investigations/exp006_timing/007_li_2025_digitized_compression_audit.md](../research/investigations/exp006_timing/007_li_2025_digitized_compression_audit.md)
- [docs/wiki/research/investigations/exp006_timing/005_li_2025_timing_overlap_matrix_plan.md](../research/investigations/exp006_timing/005_li_2025_timing_overlap_matrix_plan.md)

## Files Already Compliant

No backlink-recommendation file already linked to
[repository_levels.md](../glossary/repository_levels.md) before this pass.

## Duplicate Explanation Cleanup

Detailed duplicate level explanations were replaced with glossary, roadmap, or
gate backlinks in:

- [PROJECT.md](../../../PROJECT.md): removed local validation-order and Level
  1.5/2A/2B explanation blocks; this file now points to the glossary and
  roadmap for level terminology and status.
- [docs/00_project/research_program.md](../../00_project/research_program.md):
  removed local Level 0/1/2A/2B terminology explanations while preserving the
  historical experiment lineage.
- [docs/05_next_steps/900_roadmap_overview_ko.md](../../05_next_steps/900_roadmap_overview_ko.md):
  removed duplicated Korean Level 1.5 pass/fail and Level 2B success/failure
  criteria; the overview now points to the glossary, roadmap, and Level 1.5
  gate.

## Files Skipped

- [AGENTS.md](../../../AGENTS.md): level terms occur only in a fenced stage-prefix
  example.
- [docs/00_project/README.md](../../00_project/README.md): level term occurs
  only in frontmatter.
- [docs/00_project/experiment_record_convention.md](../../00_project/experiment_record_convention.md):
  level term occurs only in frontmatter.
- [docs/00_project/frontmatter_convention.md](../../00_project/frontmatter_convention.md):
  level terms occur only in a fenced stage-prefix example.
- [docs/02_theory/010_qfuds_v0_3_gamma_laws.md](../../02_theory/010_qfuds_v0_3_gamma_laws.md):
  level term occurs only in frontmatter.

## Broken Links Found

No broken glossary backlinks were found by `scripts/validate_docs.py`.

## Remaining Terminology Inconsistencies

- [repository_levels.md](../glossary/repository_levels.md) is the terminology
  authority.
- [000_roadmap.md](../../05_next_steps/000_roadmap.md) remains the status
  authority.
- Level terms in frontmatter and fenced examples remain unlinked because
  Markdown links there would either be invalid metadata or non-clickable example
  text.
- Code comments and generated outputs still contain level terms, but they are
  not maintained Markdown backlink targets.
- Level 0 and Level 1 still have weaker promotion/demotion criteria in the
  underlying evidence than Level 1.5 and Level 2; the glossary records that gap
  instead of inventing criteria.
