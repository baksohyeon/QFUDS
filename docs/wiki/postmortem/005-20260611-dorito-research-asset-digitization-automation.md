...
audit_log:
  - action: created
    at: 2026-06-11
    by: dorito
    note: "Recorded the follow-up incident after multi-paper asset digitization was moved from ad hoc [MarkItDown](https://github.com/microsoft/markitdown) attempts into the [PageIndex](https://github.com/VectifyAI/PageIndex) and figure-handling workflow."
  - action: updated
    at: 2026-06-11
    by: dorito
...
code_refs:
  ...
  - file: .agent/workflows/research-asset-digitization-workflow.md
    note: "New workflow created from the incident lessons: [PageIndex](https://github.com/VectifyAI/PageIndex) for paper PDFs, [MarkItDown](https://github.com/microsoft/markitdown) for non-paper assets, TeX-based figure mapping, high-resolution rendering, and coverage audit."
...
| 용어                    | 이 사건에서의 의미                                                                                                                                                                                    |
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [MarkItDown](https://github.com/microsoft/markitdown)              | PDF, HTML, TXT, JSON, notebook 등을 Markdown으로 바꾸는 범용 변환 도구. 논문 PDF 전문에는 line order(줄 순서), equation(수식), figure reference(그림 참조)가 깨질 수 있다.                            |
| [PageIndex](https://github.com/VectifyAI/PageIndex)               | 논문 PDF를 페이지/섹션 단위로 구조화해서 읽는 MCP(Model Context Protocol, 외부 도구 연결 규격) 기반 문서 처리 도구. 이번에는 paper-PDF 전문과 outline(섹션 구조) 추출 경로로 사용했다.                |
...
| figure mapping          | 논문 Figure 번호와 실제 source figure 파일을 연결하는 작업. [PageIndex](https://github.com/VectifyAI/PageIndex) caption 순서가 아니라 TeX `\includegraphics{...}`(TeX 원고의 그림 삽입 명령)가 권위 소스다.                                     |
| data-release asset      | 논문 본문이 아니라 Zenodo README, JSON metadata(기계 판독용 설명 데이터), notebook(실행 가능한 분석 노트), CSV, figure archive, code manifest 같은 release 자료. 이 경우 [MarkItDown](https://github.com/microsoft/markitdown)이 적합할 수 있다. |
...
1. [MarkItDown](https://github.com/microsoft/markitdown)으로 논문 PDF를 처리했을 때 본문 구조, figure reference, equation 탐색 품질이 충분하지 않았다.
2. [PageIndex](https://github.com/VectifyAI/PageIndex) caption 순서만 보고 figure 번호를 매핑하면서 Amendola 2024와 Farrah 2023 figure 연결이 틀렸다.
...
6. [MarkItDown](https://github.com/microsoft/markitdown)과 [PageIndex](https://github.com/VectifyAI/PageIndex)의 역할 차이를 처음부터 명시하지 않아서, PDF 논문 전문 변환과 TXT/HTML/data-release 변환이 같은 문제처럼 취급됐다.
...
| -------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| H1. [MarkItDown](https://github.com/microsoft/markitdown) 하나로 paper-PDF 전문 변환까지 충분하다                           | Markdown 파일은 생성되므로 겉으로는 성공처럼 보인다                                | paper Markdown에서 섹션 순서, figure link, equation, caption을 원문과 대조한다                        |
| H2. 논문 PDF는 [PageIndex](https://github.com/VectifyAI/PageIndex)가 기본 경로여야 한다                                    | 논문은 페이지/섹션 구조와 수식이 중요하고 PDF line order가 깨질 수 있다            | [PageIndex](https://github.com/VectifyAI/PageIndex) structure/full-text output과 [MarkItDown](https://github.com/microsoft/markitdown) 결과를 역할별로 분리한다                            |
| H3. figure 번호는 [PageIndex](https://github.com/VectifyAI/PageIndex) caption 순서로 매핑해도 된다                         | [PageIndex](https://github.com/VectifyAI/PageIndex)가 figure caption을 순서대로 보여주면 번호와 파일을 연결할 수 있어 보인다 | arXiv `.tex`의 `\includegraphics{...}`와 `\caption{...}` block을 확인한다                             |
...
| H6. txt/html/data-release 자료도 [PageIndex](https://github.com/VectifyAI/PageIndex)로 처리해야 한다                       | 모든 문서를 같은 도구로 처리하면 단순해 보인다                                     | asset을 `paper-PDF`와 `data-release`로 나누고 [MarkItDown](https://github.com/microsoft/markitdown) 적용 대상을 제한한다                         |
...
| H8. [MarkItDown](https://github.com/microsoft/markitdown)과 [PageIndex](https://github.com/VectifyAI/PageIndex)는 둘 다 Markdown을 만들기 때문에 결과 품질도 비슷하다 | 산출물 확장자가 같으면 같은 evidence state처럼 보인다                              | PDF 논문, HTML/TXT, data-release 각각에서 section order, equation, table, figure reference를 비교한다 |
...
- paper-PDF asset에는 `pageindex_structure.md`와 `paper_arxiv_*.md` 형태의 [PageIndex](https://github.com/VectifyAI/PageIndex) 산출물이 생겼다.
...
- H6도 지지된다. `gwtc4_population_release.md`, `popsummary_tutorial.md`, `zenodo_README.md`처럼 data-release 성격의 문서는 [MarkItDown](https://github.com/microsoft/markitdown)/structured conversion route가 맞다.
...
처음에는 [MarkItDown](https://github.com/microsoft/markitdown)을 중심으로 생각했지만, 사용자는 PDF 논문에서 도구 차이가 크다는 점을 계속 짚었다.
...
| 입력                       | [MarkItDown](https://github.com/microsoft/markitdown) 판단                                              | [PageIndex](https://github.com/VectifyAI/PageIndex) 판단                  | 이유                                                                                     |
| -------------------------- | ------------------------------------------------------------ | ------------------------------- | ---------------------------------------------------------------------------------------- |
...
| JSON metadata              | 사용 가능하되 표/목록으로 재정리 필요                        | 부적합                          | [PageIndex](https://github.com/VectifyAI/PageIndex)는 paper reading 도구이지 metadata normalizer가 아니다                          |
| notebook/CSV/table release | [MarkItDown](https://github.com/microsoft/markitdown) 또는 별도 table conversion                        | 부적합                          | 목적은 paper reading이 아니라 release inventory와 table inspection이다                   |
...
PDF에서 [MarkItDown](https://github.com/microsoft/markitdown)이 특히 위험한 이유:
...
[PageIndex](https://github.com/VectifyAI/PageIndex)가 더 나았던 이유:
...
하지만 [PageIndex](https://github.com/VectifyAI/PageIndex)도 figure mapping의 최종 권위는 아니다. [PageIndex](https://github.com/VectifyAI/PageIndex)는 caption과 page context(해당 페이지 주변 본문)를 읽는 데 유용하지만, Figure 번호와 실제 image file name(그림 파일명)은 arXiv source의 `.tex`에서 `\includegraphics`와 `\caption`을 확인해야 한다.

판정:

- H8은 기각된다. 둘 다 Markdown을 만들 수 있어도 evidence state가 다르다.
- [MarkItDown](https://github.com/microsoft/markitdown) PDF output은 수동 대조 전에는 `low_fidelity_search_text`로 둔다.
- [PageIndex](https://github.com/VectifyAI/PageIndex) paper output은 검토 가능한 `source_text_parse`로 둘 수 있다.
- TXT/HTML/JSON/notebook/data-release는 [MarkItDown](https://github.com/microsoft/markitdown) route(처리 경로)가 실용적이다.
...
- paper-PDF는 [PageIndex](https://github.com/VectifyAI/PageIndex) structure(섹션 구조)와 full text extraction(전문 텍스트 추출)으로 보낸다.
- data-release는 [MarkItDown](https://github.com/microsoft/markitdown) conversion(문서 변환)과 table/figure inventory(표·그림 목록 점검)로 보낸다.
- figure mapping은 [PageIndex](https://github.com/VectifyAI/PageIndex) caption 추정이 아니라 TeX source의 figure block을 기준으로 한다.
- vector figure PDF는 낮은 해상도 preview가 아니라 high-resolution PNG로 다시 렌더링한다.

판정:

- H1은 기각된다. [MarkItDown](https://github.com/microsoft/markitdown)은 paper-PDF 기본 경로가 아니다.
...
| `digitization/`      | [PageIndex](https://github.com/VectifyAI/PageIndex) outline/full text, [MarkItDown](https://github.com/microsoft/markitdown) release Markdown, table inventory 같은 derived inspection text를 둔다. |
...
| PDF parsing     | agent PDF extraction failure를 숨기지 않기 | paper-PDF는 [PageIndex](https://github.com/VectifyAI/PageIndex), data-release는 [MarkItDown](https://github.com/microsoft/markitdown)으로 분리하기              |
...
- [PageIndex](https://github.com/VectifyAI/PageIndex) MCP API 호출 로그 자체는 (편의상) Claude Cowork 로 처리해서 하위 부록으로 작성함.
...
| 결정                | 고른 것                                                            | 안 고른 대안                                      | 근거                                                                            |
| ------------------- | ------------------------------------------------------------------ | ------------------------------------------------- | ------------------------------------------------------------------------------- |
| paper-PDF 변환 경로 | [PageIndex](https://github.com/VectifyAI/PageIndex) structure + full text                                    | [MarkItDown](https://github.com/microsoft/markitdown) PDF output을 논문 전문으로 사용        | [MarkItDown](https://github.com/microsoft/markitdown)은 논문 PDF에서 section/equation/figure reference가 깨질 수 있다      |
| non-paper 변환 경로 | [MarkItDown](https://github.com/microsoft/markitdown)으로 HTML/TXT/JSON/notebook/CSV release를 Markdown화     | 모든 자료를 [PageIndex](https://github.com/VectifyAI/PageIndex)로 밀어 넣기                 | data-release는 논문 본문 구조보다 manifest/table/figure inventory가 중요하다    |
| figure mapping      | TeX `\includegraphics` block을 권위 소스로 사용                    | [PageIndex](https://github.com/VectifyAI/PageIndex) caption 순서로 추정                     | Amendola/Farrah mapping 오류가 실제로 발생했다                                  |
...
| converter 선택      | paper-PDF는 [PageIndex](https://github.com/VectifyAI/PageIndex), TXT/HTML/data-release는 [MarkItDown](https://github.com/microsoft/markitdown)          | 모든 Markdown 변환을 같은 품질로 취급             | PDF 논문과 release text는 구조 실패 모드가 다르다                               |
...
6. `paper-PDF`는 [PageIndex](https://github.com/VectifyAI/PageIndex)로 `pageindex_structure.md`와 full-text `paper_*.md`를 만든다.
7. `data-release`는 [MarkItDown](https://github.com/microsoft/markitdown)과 structured table/figure inventory로 처리한다.
...
- 권위 mapping은 [PageIndex](https://github.com/VectifyAI/PageIndex) caption 순서가 아니라 TeX source다.
- 고해상도 figure rendering은 후속 옵션이 아니라 기본값이다.
- 긴 논문은 full text default이고, 임의 축소하지 않는다.
- [MarkItDown](https://github.com/microsoft/markitdown)은 paper-PDF 기본 경로가 아니라 non-paper/data-release 경로다.
- asset은 자료 종류별 category가 아니라 paper/release key 기준으로 묶는다.
- [MarkItDown](https://github.com/microsoft/markitdown) PDF output은 기본적으로 `low_fidelity_search_text`, [PageIndex](https://github.com/VectifyAI/PageIndex) paper output은 검토 가능한 `source_text_parse`로 분리한다.
...
- 처음에는 Codex 쪽 [MarkItDown](https://github.com/microsoft/markitdown) 기반 변환이 논문 전문 처리에 충분하지 않았다.
- 특히 PDF 논문에서는 [MarkItDown](https://github.com/microsoft/markitdown)과 [PageIndex](https://github.com/VectifyAI/PageIndex)의 품질 차이가 컸고, [MarkItDown](https://github.com/microsoft/markitdown)은 TXT/HTML/data-release 쪽으로 역할을 좁혔다.
- Claude cowork에서 [PageIndex](https://github.com/VectifyAI/PageIndex) MCP(Model Context Protocol, 외부 도구 연결 규격)를 사용해 여러 논문의 structure(섹션 구조)와 full text extraction(전문 추출)을 진행했다.
- Amendola 2024와 Farrah 2023에서 [PageIndex](https://github.com/VectifyAI/PageIndex) caption 순서 기반 figure mapping이 틀렸고, TeX `\includegraphics` 확인으로 수정했다.
...
- TXT/HTML/JSON/notebook/data-release 성격 자료는 [MarkItDown](https://github.com/microsoft/markitdown)이 적합한 경로로 분리됐다.
...
- paper-PDF asset은 [PageIndex](https://github.com/VectifyAI/PageIndex) route(논문 PDF 처리 경로)를 탄다.
- TXT/HTML/JSON/notebook/data-release asset은 [MarkItDown](https://github.com/microsoft/markitdown) 또는 structured conversion route(구조화 변환 경로)를 탄다.
...
- figure mapping은 [PageIndex](https://github.com/VectifyAI/PageIndex) caption 순서가 아니라 source TeX를 우선한다.
...
- TeX가 draft superset(출판본보다 많은 draft용 그림이 남은 원고)이면 compiled PDF(조판된 최종 PDF)나 [PageIndex](https://github.com/VectifyAI/PageIndex) page context(페이지별 본문 내용)로 대조한다.
...
- 일반: [MarkItDown](https://github.com/microsoft/markitdown)이 TXT/HTML/JSON/notebook/CSV 등을 Markdown으로 변환한다.
...
- 이번 용도: [PageIndex](https://github.com/VectifyAI/PageIndex) caption 순서 추정으로 틀렸던 amendola/farrah를 TeX 기준으로 교정. 단 `Cosmological Budget of Entropy from Merging Black Holes`처럼 draft superset이면 compiled PDF와 교차검증.

### 긴 논문 full-text 추출 / 조립

- [PageIndex](https://github.com/VectifyAI/PageIndex)(MCP 도구): `process_document`(적재) -> `get_document_structure`(outline, 섹션 구조) -> `get_page_content`(페이지 본문). `get_page_content`는 응답이 약 50KB를 넘으면 temp 파일로 persist(임시 파일로 저장)되므로 6–8 페이지 단위 chunk(페이지 묶음)로 부른다.
...
이런 부류의 작업은 보통 이 순서로 푼다: 압축 해제 → (paper면 [PageIndex](https://github.com/VectifyAI/PageIndex) / release면 [MarkItDown](https://github.com/microsoft/markitdown)) 변환 → TeX로 figure 매핑 확정 → 고해상도 렌더 → 커버리지 감사 → validator. 매 단계 산출물은 `source_text_parse`까지만, numerical product 아님.
