...
- 분리 figure 파일이 없는 논문도 있다. Lineweaver entropy-budget chapter(`Chapter22Lineweaver.pdf`)는 책 챕터 PDF라 source에 별도 figure 파일이 없었다. PDF embedded raster(PDF 안에 박힌 사진 이미지, 예: M87 사진)는 `pdfimages`로 native image(원본에 가까운 추출 이미지)를 뽑아 그대로 두고, vector diagram(선과 글자로 그려진 도식, Fig 22.2~22.4)은 `pdftoppm`로 high-DPI page render(고해상도 페이지 이미지)를 만든 뒤 crop(필요한 영역만 자르기)해야 했다. 즉 "figure 파일이 없다"가 "figure를 못 만든다"는 아니다.
...
- 이번 용도: data-release의 release note, event-list, CSV 표를 Markdown으로 — `paper-PDF`가 아니라 `data-release` 경로에서만.
...
- 일반: `pdfimages`는 PDF에 박힌 raster image(픽셀 이미지)를 그대로 뽑고, `pdftoppm`은 페이지 전체를 raster로 렌더, `convert -crop/-trim`은 그중 figure만 잘라낸다, `identify`는 픽셀 크기를 본다.
...
- PageIndex(MCP 도구): `process_document`(적재) -> `get_document_structure`(outline, 섹션 구조) -> `get_page_content`(페이지 본문). `get_page_content`는 응답이 ~50KB를 넘으면 temp 파일로 persist(임시 파일로 저장)되므로 6~8 페이지 단위 chunk(페이지 묶음)로 부른다.
