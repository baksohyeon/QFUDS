...
### 5.1 심각도: 결론이 바뀌는 오류였나?

짧은 판정은 이렇다. **최상위 연구 결론이 뒤집힐 정도는 아니었지만, 증거의
엄밀성을 위해서는 꽤 critical했다.**

바뀌지 않은 결론:

- QFUDS가 검증된 물리 이론이 됐다는 결론은 여전히 없다.
- rough-tanh lineage는 여전히 `proxy / phenomenology / observer mode`다.
- Level 3 `CLASS/hi_class` 검증은 여전히 blocked다.
- P1은 안정해도 새 QFUDS 물리가 아니라 IV/IDE 현상학이다.
- P2는 retained amplitude에서 여전히 실패한다.
- `c_eff²`, `c_vis²`, `ξ≈10 Mpc`는 여전히 tuning/parameterization 쪽 결론이다.

오류별 심각도:

| 항목 | 수치/증상 | 결론 영향 | 왜 critical했나 |
| --- | --- | --- | --- |
| background normalization | 기존 합은 `Ω_total=1.000092`라 `H(a=1)` 오차가 약 `4.6e-5`, 즉 약 0.005% 수준이었다. | 연구 결론을 뒤집지는 않는다. | baseline invariant라 tolerance 안에서 고정돼야 한다. 작은 오차라도 기준선 검산에는 critical하다. |
| P2 `abs(rho_B)` | 음수 B상 밀도를 denominator에서 숨길 수 있었다. | QFUDS를 살리는 방향으로 결론을 바꾸지는 않았다. | 실패해야 할 branch가 "계산된 것처럼" 보일 수 있어 증거 관리상 critical하다. |
| conservation residual | 계산하지 않은 값이 0처럼 남을 수 있었다. | 물리 결론을 뒤집지는 않는다. | 0은 보존 검증 통과처럼 읽힌다. 미계산 값은 `NaN/status`로 남겨야 한다. |
| CP16 scale band | 그림의 scale-dependence band가 scale factor grid를 redshift 값으로 보간했다. | 핵심 CSV/수치 결론은 유지된다. | 그림이 독립 지문처럼 읽히므로 시각 증거의 엄밀성에는 critical하다. |
| CP15 해석 | "high-k onset" 설명이 실제 ratio shape와 맞지 않았다. | 점성이 독립 knob라는 결론은 유지된다. | 잘못된 물리 직관을 문서가 강화할 수 있어 해석상 critical하다. |

따라서 이번 수정은 QFUDS를 더 살린 것이 아니라, 실패와 한계 판정을 더 방어 가능하게
만든 작업이다. 방치했다면 나중에 "증거보다 강한 주장"으로 오염될 수 있었지만, 현재
수정 후에는 어느 값이 계산됐고 어느 부분이 proxy인지 더 명확해졌다.
