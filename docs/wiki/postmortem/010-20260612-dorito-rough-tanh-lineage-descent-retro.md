...
| CP14 | 두 신호 실오차 채점 | w(z) freezing 대표 DESI서 5.5σ·반대 사분면, P(k) 스텝 Stage-IV 약 18σ (외부 숫자=예시) |
...
| CP19 | Euclid forecast | P(k) 스텝 다중-bin 약 24σ, γ running 약 24σ 구별. 균일 이동 0σ(sanity) |
...
한쪽 편향(미시 foam 척도만 늘어놓은 것)을 잡아냈고, 인과/Kibble 지평(약 4400 Mpc,
...
자유 파라미터가 줄어드는지, 아니면 자리만 옮겨가는지다.* CP21(ξ를 근임계성이나
