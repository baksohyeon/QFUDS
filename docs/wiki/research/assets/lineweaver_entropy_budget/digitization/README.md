---
doc_id: asset_lineweaver_entropy_budget_digitization_index
title: Lineweaver Entropy-Budget Digitization Assets
doc_type: index
stage: reference
status: reference
...
chapter has no figure PNG mirrors in this cache, so its four figures are recorded
as captioned placeholders.

For entropy-inventory work, manually extract only the needed table rows,
captions, or figure values from the source PDF or rendered pages, then store the
curated output here.
