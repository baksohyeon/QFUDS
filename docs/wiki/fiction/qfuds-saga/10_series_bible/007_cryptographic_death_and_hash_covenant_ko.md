...
사용자 노트의 기술 개념을 아래처럼 이야기 장치로 번역한다.
...
KDF는 비밀을 여는 비용으로 번역한다. SAGA에서 이 비용은 계산 시간이 아니라
제도, 대기, 접근권, archive 점유권으로 나타난다.
...
HashDoS는 Identity Flood로 번역한다. 정상 시스템이 너무 많은 collision을
받으면 최악의 경우로 떨어진다는 구조만 가져온다.
...
2. "모든 암호가 다 뚫렸다"는 문장은 피한다. 대신 어떤 종류의 비대칭성이
   어떤 제도에서 무너졌는지 보여준다.
3. hash, KDF, key, salt, collision은 권리, 비용, 신원, 절차로 번역한다.
