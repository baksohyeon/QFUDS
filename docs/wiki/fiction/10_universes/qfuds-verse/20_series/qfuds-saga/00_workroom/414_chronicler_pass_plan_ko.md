...
= design(승격 전 stable canon 아님). `10_world/117-122·123·124`·`story_design/210`·`309 brainstorm`
...
| 028_qday_world_system_14domain_matrix_ko | qfuds_saga_qday_world_system_14domain_matrix_ko | guide | canon(115 부속) |
| 117-035_world_expansion_wave1-6 | qfuds_verse_world_expansion_wave1-6_* | guide | **candidate**(공유 세계 대사전) |
...
401 창작 시스템 · 102 용어 글로서리 · 403 2부 GSD brief · 103 제작 하네스 · 104 아이디어 추적 ·
406 1부 Book1 GSD brief · 407 갭 감사 · 408 Production Board · 409 Intent Card 템플릿 ·
410 패널 인계 · 203 근미래 예측 방법 · 412 리서치 앵커 · 413 Truth-State Ledger · README.
(doc_id는 `qfuds_saga_*`. 104·201·105은 legacy 번호 주의, §6.)

### 1.5 `10_story_design` (23) 전부 design (승격 전 stable canon 아님)

001 시각 전시 · 105 Arc Two 계획(legacy명) · 303 반전 설계 · 304 형식·throughline ·
305 2부 에피소드 맵 · 306 아크 지도(부 번호 remap SSOT) · 307 1부 outline reboot(status=completed) ·
308 씬 카드(status=completed) · 309 sovereign-AI(**candidate/brainstorm**) · 310 5대 질문 스파인 ·
311 origin 아웃라인 · 312 origin 씬 카드 · 111 암호 온보딩 · 206 사엘 실행 시트 · 315 bridge ·
316 리빌 사다리 · 317 causal master · 318 오리엔테이션 · 319 오르페우스 설계 · 320 근미래 리센터 ·
321 기계 화자 · 210 근미래 프렐류드(**candidate**) · README.
...
000 §도메인 권위 표 + 123 경계에서 확정한 도메인별 최종 권위:

```text
000 (권위 루트: 명시 우선 > 도메인 표 > last_updated > unknown)
├─ 복원 물리      → 113  (그 위 원본 부재 물리 114가 최상위)
├─ 명칭·세력명     → 109
├─ Q-Day 여파     → 115 (부속 116)
├─ 인과·Last Archive → 107
...
└─ 이념 형이상학    → 123 (candidate, 114·115 진자 위)
```

충돌 우선순위(000·123): **114 > 113 > 115 > 102 > 109**. 피의존 키스톤 = 107(in-degree 18).
...
| # | 드리프트 | 근거 위치 | 상태 |
| --- | --- | --- | --- |
| D2 | "부활/완전 복원" 현대 gloss | 113 §5(line 126-128), 001 §배너(line 24-29), 103 §목적 배너(line 24-30) | **해소**: 124 원어 층 + 배너 |
| D6 | "1부=Cryptographic Death"·"사엘=1부" | 001 line 24 이하 1기 서술 | Phase 2 |
| DRIFT-008/017/020 | legacy 부 라벨(arc two, 사엘=1부, Mara=1부) | 202 §Arc Two Hook(line 182-239), 302 title·§Arc Two(line 3·17·53·77), 307 line 94, 308 line 148, **209 line 108 "사엘(1부)"** | Phase 2 (024 최우선) |
| DRIFT-009/012 | 201 Naming Table가 109와 어긋날 소지 | 201 line 187 `Castra Tabularii`, 109 §명칭 충돌(line 87-95·139) | Phase 1 (검증 필요) |
| DRIFT-013 | 206 Core Drive가 203·205 반복(20-30%) | 206 §Identity 이하 | Phase 3(참조화) |
| DRIFT-011 | Migration compatibility layer(배너로만 차단) | 202·203·205·206 상단 배너 | Phase 2 전체가 이 부채 해소 |
| DRIFT-005/021 | candidate 과잉승격 위험 | 030-035, 210, 123, 124 | Phase 4(라벨·게이트) |
| DRIFT-022 | 208 "복제품" 표현 | 208 line 88·91·157 | 독자용 허용, canon 문서는 사본/재구성체 |

주의(검증 필요): **209 line 108 "사엘(1부)"**는 시대 좌표 SSOT 본문이 현재 매핑(사엘=1.5부)과
어긋나는 지점이다. SSOT 자체의 라벨이므로 Phase 2에서 가장 먼저 처리한다. 단, 209가 "1부"를
포괄 의미(사엘 계열 = 넓은 1부군)로 쓴 것인지 엄밀히 1.5부 오기인지는 저자 확인이 필요하다(미정).

## 5. Candidate Promotion Register

전부 candidate. 승격은 [117 §7 6체크 게이트](../../../10_world/117_world_expansion_wave1_names_places_events_ko.md)
+ [901 §4](../../../00_continuity/901_canon_drift_and_tech_debt_report_ko.md) 게이트.

| candidate | 위치 | 승격 타깃 SSOT | 특이 리스크 |
| --- | --- | --- | --- |
| 117-122 세계 확장 웨이브 | 10_world | 명칭→109, 세력→102, 여파→115 | Vera/Vera Dace 충돌(118, 개명 권장) |
| 123 이기 이념 축 | 10_world | 102·115 진자·114 물리 | 실존 성리학 비대응 유지 |
| 124 먼 미래 원어 층 | 10_world | 113·114·123 | 라틴은 기존 제도 관습 상속(121), 신규 conlang 아님 |
| 210 근미래 프렐류드 | story_design | 321·301·bible-210 | 과잉밀도(DRIFT-113), 6체크 개별 승격 |
| 309 sovereign-AI 축 | story_design | 317·318 | brainstorm, 선택 씨앗만 |

6체크(승격 전 필수): (1) 109 명칭 충돌 없음, (2) 114/113 물리 모순 없음, (3) 115 여파 착지,
...
| --- | --- | --- | --- |
| Domus Registri / Domus Tabularii / House Tabularii | **Domus Clavium** (House of Keys) | 109 line 87-95·139, 000 line 40, 205 line 109 | 201 line 187 `Castra Tabularii`(원장 성채=Ledger Keeps): 어근 Tabularii 잔존. 지명 의도인지 검증 필요 |
| "완전 복원 / 완전 역연산 / 부활(원본 회복)" | 재현·N세대 귀환·해상 손실 (124), 기전=사본(113) | 113 §5, 124 대응표 | 001·103 배너로 gloss 처리됨(해소). 잔여 canon 본문 naive 표현 없음 |
| "복제품"(독자 설명어) | 사본 / 재구성체 | 114·113 정밀 구분 | 208 line 88·91·157: 독자 오리엔테이션은 허용, canon 문서 확산 금지 |
| legacy 부 라벨: "Arc Two / 아크 투 / 사엘=1부 / Mara=1부" | 209 매핑: 0부 캐스 / 1부 오르페우스 / 1.5부 사엘 / 2부 마라 / 3부 author-loss | 209, 320, 306 §부 번호 remap(line 277) | 302·202·307·308·209 line 108 (§4 D6·DRIFT-017) |
...
원칙: SSOT 우선순위(114>113>115>102>109) 보존. 한 Phase = 한 커밋. drafts/revisions/archive
미수정. 산문·본문에 손대므로 **각 Phase 착수 전 사용자 승인**. 배너는 본문 정합이 끝난 뒤에만 걷는다.

- **Phase 0 준비:** 대상 파일 목록 확정, `git status` 청결 확인, 이 계획서 승인. 편집 없음.
- **Phase 1 명칭 잔재(최저 위험):**
  - 201 line 187 `Castra Tabularii`가 지명(Ledger Keeps)으로 의도된 것인지 확인. 109 단일화와
    충돌이면 라틴명 정합, 지명 고유라면 109에 예외로 등록. (미정 → 저자 확인)
  - 201 Naming Table 전반을 109 기준으로 대조, 폐기 표기 잔재 정리. Domus Clavium 유지.
- **Phase 2 부 라벨 정합(핵심, DRIFT-011 해소):**
  - **209 line 108 "사엘(1부)"를 최우선**으로 209 매핑(1.5부)에 맞춰 정합(또는 포괄 의미 명시).
  - 202(§Arc Two Hook·line 111-239)·105(title·본문)·203·108(completed)·story_design README line 65의
    legacy "Arc Two / 사엘=1부"를 209·306 remap 기준으로 치환. stable ID 정책상 파일명·doc_id는
    유지, 본문 라벨만 정합. 정합 완료 문서에서 상단 리센터 배너를 축약·제거(compatibility layer 해소).
  - 001 1기 서술(D6)은 124 원어 층·현대 gloss 프레임 유지가 이미 되어 있으므로 부 라벨 표기만 점검.
- **Phase 3 캐릭터 참조화:** 206 반복 Core Drive를 "203·205 참조"로 축약(병합 아님). 시대좌표·
  원본/사본 상태 서술은 209로만.
- **Phase 4 candidate 위생:** 117-122·210·123·124·204에 candidate 배너·6체크(§5) 링크 확인/보강.
- **Phase 5 위생·마감:** status 안정 문서 reference 승격 검토, 029 결번 각주(000/README), 900·901·109
  상호 정합 갱신. 검증 스위트 통과 후 커밋.

각 Phase 후 900/901/413을 갱신해 원장과 본문이 어긋나지 않게 한다.
