...
### B2. 로런츠 대칭(근본 vs 창발)·시간 화살·텔레포테이션 (025 substrate)

- **주류**: 로런츠 불변은 초정밀 검증(Δc/c ≲ 10^-17). 불변 속도는 시공간 대칭의 성질이지
  빛이 특별해서가 아님. 시간 화살 = 법칙이 아니라 **낮은 엔트로피 초기조건**(Past Hypothesis,
  Penrose Weyl). 텔레포테이션 = 원본 파괴 후 재구성(사본 아님, 노클로닝 위반 아님), **025·021의
  복원=사본·재구성 근거의 정론.**
...
### B3. 화이트홀 바운스·잔해·우주상수 (White Hole 스레드·036·DM↔DE)
...
4. 물리 substrate는 025·036·White Hole 스레드에서 in-world 사변으로만. QFUDS 연구 status·
...
이 대장은 근미래 프렐류드(story_design 027)와 in-world 물리(025)의 핍진성 닻이다. 실제 예측·
