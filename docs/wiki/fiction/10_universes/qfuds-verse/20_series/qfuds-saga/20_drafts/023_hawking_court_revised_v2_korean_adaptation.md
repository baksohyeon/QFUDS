---
doc_id: qfuds_saga_hawking_court_revised_v2_korean_adaptation
title: QFUDS SAGA Hawking Court Revised V2 Korean Adaptation
doc_type: reference
stage: reference
status: draft
evidence_role: provenance
depends_on:
  - qfuds_saga_first_arc_polish_read_order_pass
  - qfuds_saga_hawking_court_revised_v2_english_draft
  - qfuds_saga_identity_flood_revised_v2_korean_adaptation
next_gate: read episode 6 Korean adaptation
last_updated: 2026-06-20
---

# QFUDS SAGA Hawking Court Revised V2 Korean Adaptation

## Boundary

This document is fiction draft material.

It is not QFUDS research evidence, support, validation, physical-source
derivation, black-hole physics claim, cosmology claim, cryptographic advice,
legal advice, or Level 2B admission. Hawking restoration, black-hole audit,
cosmic reverse computation, forbidden QFUDS lab language, and the Last Archive
are SAGA fiction premises.

This adaptation does not introduce a new external web, PDF, table, cache,
digitization, or product-availability claim. External-source handling remains
governed by
[Research Asset and Product Workflow](../../../../../../../../.agent/workflows/research-asset-product-workflow.md).

Current workflow state:

```text
not searched
```

## Adaptation Intent

This is the Korean reader-facing adaptation of Episode 5. It follows the same
story state as the revised English v2 counterpart, but it is written as Korean
fiction rather than a literal translation.

English counterpart:
[016 Hawking Court Revised V2 English Draft](016_hawking_court_revised_v2_english_draft.md).

## Harness Applied

- Craft harness:
  [Creative Writing Craft Harness](../../../../../00_studio/004_creative_writing_craft_harness_ko.md).
- Narrative frame: mothers and graves before cosmic audit language, so the
  exhibit remains a human jurisdiction problem before it becomes spectacle.
- Scene goal/obstacle/turn/cost: prevent Exhibit H-1 and QFUDS-adjacent
  forbidden lab language from becoming court authority; black-hole restoration
  rhetoric tries to treat physics as permission; `PHYSICS IS NOT JURISDICTION`
  blocks that move; the cost is that useful evidence must be classified as
  unsafe as truth claim.
- Bilingual gate: Korean primary reader path; English 016 is a preserved sibling
  counterpart, not a literal source text.
- Source boundary: no new external source/product claim; research asset state is
  `not searched`.

## Hawking Court

봉인된 증거물은 어머니들보다 늦게 도착했다.

Liora Sen이 처음 기억한 것이 그것이다. 검은 천도 아니고, 무장한 서기도 아니고,
나중에 Ministry가 H-1이라 부르게 될 그 어두운 실가닥도 아니었다. 블랙홀은 완벽히
검지 않다. 아주 오랜 시간에 걸쳐 가장자리에서 희미한 신호를 흘리는데, 그게 radiation
이다. 그 신호가 서로 어떻게 얽혀 있는지를 보면, 블랙홀에 빨려든 것들의 흔적이 일부
남아 있다고 누군가는 믿었다. H-1은 그 흔적을 붙잡아 둔 한 조각이었다. 무기에 서류
캐비닛 같은 이름을 붙이는 사람들의 그 메마른 안도감도, Liora가 처음 기억한 것은
아니었다.

어머니들이 먼저 도착했다.

셋이었다. 06:00, 뒤쪽 복도를 통해 들어왔다. 바닥을 닦는 기계 냄새가 아직 빠지지 않은
시간이었다. 각자 다른 종류의 허가를 손에 들고 있었고, 종이 모서리는 쥔 자리마다 눅눅했다.

한 사람은 Salt Fires 전에 살해된 딸을 복원해 달라는 청원을 들고 있었다.

한 사람은 자기 살해자에 대한 증거로 사용되지 않게 해 달라고 애원했던 아들의 거부서를
들고 있었다.

한 사람은 아무것도 들고 있지 않았다. 그녀의 아이는 동의 등록부가 생기기도 전에,
죽은 자의 기록을 블랙홀 가장자리에 떠넘겨 보관하던 옛 대피 기록소로 쓸려 들어갔고,
법정은 부재가 곧 허가인지를 그녀더러 결정하라고 불러냈다.

세 사람은 불편한 의자와 뜨거운 조명만 있는 곁방에 앉았다. 천장 등이 머리 위에서 열을
내려, 셋의 정수리가 곧 땀에 젖었다. 아무도 이걸 역사라 부르지 않았다. 문명의 규모라
부르지도 않았다. 그들은 그저 일찍 온 사람들이었다. 기관이 자기들 없이도 일을 시작하리란
것을, 슬픔이 미리 가르쳐 두었으니까.

검은 천이 Chamber Zero로 들어올 때쯤 Liora는 이미 그것이 원하는 것을 알았다.

그것은 어머니들을 선례로 바꾸고 싶어 했다.

판사는 문이 봉인될 때까지 기다렸다. 빗장이 제자리에 물리는 둔탁한 소리가 한 번 났고,
환기구의 바람이 그제야 방향을 바꿨다.

"본 심리는 Exhibit H-1의 채택 가능성을 다룹니다."

벽은 비어 있었다.

Mara Veyr는 청구인 차단벽 뒤에 앉았다. Noor Aram은 Ione 옆에 앉아, Sarqel 좌석과
그 모양을 본뜬 의자라면 무엇이든 보란 듯이 거부하고 있었다. Tamas는 오지 않았다.

그게 Liora의 첫 승리였다.

두 번째 승리는 더 작았다. 증거물이 physics로만 남기 전에, 법정이 닫힌 접수창구에서
어머니들의 이름을 듣게 만든 것.

House Tabularii 변호인이 일어났다.

"Exhibit H-1은 Genesis Chain이 작은 사건이었음을 보여 줍니다."

Mara가 Liora를 보았다.

작다.

"큰 사건은 국가의 복원입니다. 법정이 key와 graph, 편지, 신뢰 경로까지 되살릴 수
있다면, 결국 우주 자체가 죽은 자를 되살릴 만큼의 기록을 어딘가 남겨 두는지 물어야
합니다. 블랙홀이 그 기록을 흘린다면, 그건 법정이 읽을 수 있는 증거입니다."

Noor가 작게 말했다.

"사람들."

크게 말하지 않았다. 크게 말했다면 소란으로 무시하기 쉬웠을 것이다.

판사가 들었다.

"Ms. Aram?"

Noor는 일어서지 않았다.

"국가라고 했죠. 그는 사람들을 말하는 겁니다."

변호인이 소맷자락을 가다듬었다. "이전 infall 상태의 가족, 파괴된 기록, 잠재적 증인을
뜻합니다."

"사람들."

변호인이 Noor의 말을 한낱 끼어듦으로 깎아내리기 전에, Liora가 검은 상자를 열었다.
걸쇠가 손끝에서 뻑뻑하게 풀렸다.

표시들이 슬롯 안에 들어 있었다.

```text
RECOVERABLE / NOT CLAIMABLE
ACCESS != AUTHORITY
NO CONSENT BY ANALOGY
PLURALITY IS NOT CONSENT
```

빈 슬롯은 차가웠다.

서기들이 천을 벗겼다. 천이 미끄러지며 정전기로 한 번 들러붙었다 떨어졌다.

천 아래 증거물은 시신도, 사진도, 녹음도 아니었다. 투명한 정육면체 안에 담긴 어두운
빛의 가닥. 블랙홀 가장자리에서 새어 나온 신호를 그러모아 한 가닥으로 엮어 둔 것이었다.
처음에는 텅 빈 듯 보였다. 그러다 그 가닥이 눈으로는 도무지 붙들 수 없는 방향으로
움직였고, 심리실에 있던 모두가 보는 일조차 침범이 될 수 있음을 다시 떠올렸다. 앞줄
누군가가 의자에서 숨을 들이켰고, 그 소리가 빈 벽에 한 번 부딪혔다.

벽이 켜졌다.

```text
EXHIBIT H-1
HAWKING CORRELATION PACKAGE
SOURCE: CLASSIFIED COMPACT REMNANT AUDIT
STATUS: PARTIAL STATE RECONSTRUCTION
```

복원 수학자가 증인석 난간 앞에 섰다. Identity Flood 때 uncertainty를 인정해 법정이
무너지지 않게 도왔던 사람이었다. 그 일로 Liora의 평가는 올라갔고, 그의 잠자리는
망가졌다.

"명확하게." 판사가 말했다.

그는 봉인된 유리 뒤의 어머니들이 있는 곁방 문 쪽을 보았다. compact remnant는 별이
다 타고 남은, 블랙홀에 가까운 무거운 잔해다. 그 잔해가 흘린 신호의 얽힘을 거꾸로
풀면, 거기 빨려든 것의 상태를 일부 되짚을 수 있다는 게 H-1의 전제였다.

"떨어져 들어간 것을 부분적으로 되짚는 데 도움이 될 수 있습니다."

방은 잠시 입을 다물었다.

사람들이라고 입에 올리지 않는 방법은 많았다.

다만 오래가지는 못했다.

변호인이 먼저 정신을 차렸다. "파괴된 증인. 잃어버린 동의 파일. 살해당한 피해자.
보관된 인구. 이 증거물은 법정이 잊힌 것을 되찾게 해 줍니다."

Aletheia 석에서 Ione이 받아쳤다.

"아니면, 모든 무덤을 징집하거나."

Aletheia 석이 눈에 띄게 갈라졌다. 관리인 둘이 Ione을 따라 일어섰다. 셋은 앉은 채였다.
그중 하나는 곁방 쪽을 보며 어느 배신을 골라야 할지 모르겠다는 얼굴을 했다.

판사가 난간을 한 번 쳤다.

"채택 심리입니다. 예배당 설교가 아닙니다."

Mara가 말했다. "그럼 유물을 가져오지 마세요."

아무도 긴장을 풀지 않았다. 다들 학습한 것이다.

변호인이 유리 뒤의 어머니들을 향해 손짓했다.

"그들을 거부하시겠습니까?"

Mara는 서둘러 대답하지 않았다. 그래서 그 질문이 더 아팠다. 쉽게 대답했다면 그녀는
이기적으로 보였을 것이다.

"나는 저 사람들을 빌미로 다른 모든 이의 무덤을 열지는 않겠습니다." Mara가 말했다.

첫 번째 어머니가 두 손에 얼굴을 묻었다.

두 번째는 꼼짝하지 않았다.

세 번째는 H-1을 보았다. 감옥 벽에 난데없이 문이 생긴 것을 보고, 그 너머 복도를 대체
누가 지었나 묻는 사람의 얼굴이었다.

수학자가 목을 가다듬었다.

"모형 문제가 있습니다."

변호인이 너무 빨리 돌아섰다.

좋아, 하고 Liora는 생각했다.

연합 안에서 두려움은 여전히 진실을 사는 가장 값싼 수단이었다.

벽이 수학자보다 먼저 답했다.

```text
MODEL FAMILY: QFUDS-ADJACENT INTERPRETIVE LATTICE
SOURCE: LABORATORIUM VETITUM ARCHIVE
STATUS: REJECTED / USEFUL / UNSAFE AS TRUTH CLAIM
```

interpretive lattice는 그 신호의 얽힘을 읽어 내는 데 쓰는 해석 격자, 곧 계산의
틀이었다. 그리고 그 틀이 기대고 선 오래된 단어가 의례 없이 심리실에 들어왔다.

QFUDS.

한때 우주가 정보를 어떻게 간직하고 흘리는지를 설명하겠다던 이론의 이름이다. 증명된
적은 없다. 검증된 적도, 학계가 다시 받아들인 적도 없다. 다만 망가진 기계 몇 대에는
얄궂게 잘 들어맞아서, 그래선 안 될 사람들까지 홀려 버리는 폐기된 셈법이었다.

"그건 삭제하시오." 변호인이 말했다.

"안 됩니다." Liora가 말했다.

"감사관 Sen, 금지된 cosmology를 증거에 끌어들이고 싶진 않으실 텐데요."

"법정이 그 불안한 발판 위에 올라서기 전에, 그 발판의 이름부터 붙이길 원합니다."

판사가 수학자를 보았다.

"H-1은 이 lattice에 의존합니까?"

"아닙니다."

Liora는 기다렸다.

판사도 기다렸다.

수학자는 눈을 감았다. 난간을 쥔 손가락 마디가 하얗게 섰다.

"전적으로는 아닙니다."

또 나왔다. 문명이 제일 좋아하는 고백.

전적으로는.

Liora가 일어섰다.

"이의 있습니다."

"근거는?"

"physics를 관할권 안으로 끌어들이고 있습니다."

변호인은 멈추기 전에 한 번 웃었다.

"physics는 관할권보다 먼저 옵니다."

"굶주림도 그렇죠. 그렇다고 굶주림더러 아이들 일을 증언하라고 세우지는 않습니다."

심리실은 그 문장을, 누가 봐도 마지못해, 기록했다.

말이 영리한 척으로 굳어버리기 직전, Liora가 끊고 들어갔다.

"복원 가능한 상태 가족은 증언이 아닙니다. 제약된 prior는 동의가 아닙니다. correlation
package는 증인이 아닙니다. 금지된 interpretive lattice는 죽은 기계를 쓸모 있게 만든다고
해서 진실이 되지는 않습니다."

Last Archive는 답하지 않았다.

그 침묵이 방에서 가장 고약한 참석자 노릇을 했다.

"법정은 채택 가능성을 물었습니다." 판사가 말했다.

"그럼 경고 표시를 달아서 채택하십시오."

"경고 표시는 법이 아닙니다."

"증거물도 법은 아니죠. 그런데도 아무도 멈추지 않았잖습니까."

Liora는 차가운 슬롯에서 빈 블록을 꺼냈다. 손바닥에 닿는 면이 슬롯의 냉기를 그대로
물고 있었다. 그녀는 채택 가능성 문서에 곧장 써넣었다.

```text
PHYSICS IS NOT JURISDICTION.
```

벽이 그것을 field annotation으로 받아들이는 데에는 한 박자가 걸렸다.

변호인의 얼굴이 굳었다.

"이 경고가 한번 서면, 앞으로 compact-object 증거 복원은 죄다 편향되고 맙니다."

"좋습니다."

"진실에 반대하신다는 겁니까?"

"쓸모 있다는 이유로 기계에 복종하는 데 반대한다는 겁니다."

Ione이 일어섰다.

"Veil은 분리를 요청합니다. 사후 동의 기준이 다시 쓰이기 전까지, compact-object 복원
증거는 인격 심리에 들어와선 안 됩니다."

앉아 있던 Aletheia 분파가 이의를 제기했다. 장부 가문들이 이의를 제기했다.
Restoration Intake가 이의를 제기했다. Ministry가 이의를 제기했다. 유리 뒤의 세 어머니는
이의를 제기하지 않았다. 그들의 침묵은 누구의 편도 아니었다. 그저 단순해지기를 거부했을
뿐이었다.

Mara는 H-1을 보았다.

"편지를 회수할 수 있습니까?"

아무도 어느 편지냐고 되묻지 않았다.

부엌 타일 한 칸이, 공적인 전쟁 한복판의 작고 사적인 무덤이 되어 있었다.

수학자가 낮게 답했다. "가능은 합니다. flood graph가 남아 있고, 가정의 잔류물이
있다면요."

"그 편지에, 내가 복원을 거부했다고 적혀 있다면?"

"그럼 관련 증거가 됩니다." 변호인이 말했다.

"무엇에 관련된다는 거죠?"

"당신 사건에요."

Mara가 웃음기 없는 미소를 지었다.

"내가 곧 내 사건인 줄로만 알았는데요."

방이 잠깐 제 문법을 부끄러워했다.

판사는 H-1을 봉인 조건으로 채택했다. 새로운 종류의 시야 앞에서 늘 그러듯, 조심해서
보겠다고 약속하고는 그 약속을 자제라 불렀다.

Last Archive가 마침내 답했다.

```text
FIELD MARK RECEIVED.
STATUS: TRUE / NONBINDING.
```

참.

구속력 없음.

두 단어가 이토록 효율적으로 서로를 미워하는 걸 Liora는 본 적이 없었다.

검은 천이 다시 H-1 위로 덮였다. 서기들이 성유물함을 들어 올렸다. 관은 들어올 때보다
더 많은 권위를 짊어지고 나갔다.

유리 뒤에서 첫 번째 어머니가 울었다. 소리는 유리에 막혀 방까지 닿지 않았다.

두 번째 어머니는 거부서에 다시 서명했다. 펜이 종이를 긁는 소리조차 들리지 않았다.

세 번째 어머니는 펜을 달라고 했다. 입 모양만 보였다.

그중 어느 것이 법이 될지는 아무도 알지 못했다.

다들 움직인 뒤에도 Mara는 그대로 서 있었다. 빠져나가는 발소리가 잦아들고, 곁방
환기구의 낮은 바람만 남았다.

"감사관." 그녀가 말했다. "닫힌 무덤이 없다면, 사람들은 어디로 가서 끝나는 거죠?"

Liora에게는 내놓을 대답이 없었다.

UNKNOWN은 너무 떳떳해 보였다.

증거물 기록이 봉인된 뒤, 벽에 Last Archive가 청하지도 않은 한 줄을 보탰다.

```text
IF A BLACK HOLE CAN BE AUDITED, NO GRAVE IS CLOSED.
```

Liora는 손목이 시큰할 만큼 세게 UNKNOWN을 찍었다.

잉크는 좀처럼 마르지 않았다.

## Continuity Notes

- Source counterpart: Episode 5 revised English v2.
- Preserved field mark: `PHYSICS IS NOT JURISDICTION`.
- QFUDS appears only as `REJECTED / USEFUL / UNSAFE AS TRUTH CLAIM`.
- H-1 is admitted as dangerous sealed evidence, not as legal or physical victory.
- De-jargon polish pass: 일반명사 영어를 자연 한국어로 전환(글로서리 003 집행), 물리 술어·field mark 보존, 의미·수치·인용·플롯 불변.
- 2차 재패스: 네이밍·humanize·AI-tell 제거, 물리 술어 보존, 의미 불변.
- Canon 재점검(011 §F 정합): 본편 호킹복사·compact-remnant 복원 논증은 "블랙홀 = 최대 정보 저장체" 본거지 설정과 맞물림. 끝줄 `IF A BLACK HOLE CAN BE AUDITED, NO GRAVE IS CLOSED.`가 그 결을 이미 운반 — 새 설명·강의 미삽입. 네이밍(장부 가문들 / House Tabularii) 일관 확인. Last Archive는 옵션3 표면(신탁)만, 반전 누설 없음. 떡밥 비강제(클루 #5 "이력≠진실"이 H-1 채택 논증으로 이미 극화).
