...
depends_on:
  - qfuds_saga_index_ko
next_gate: keep current story-order set listed; archive superseded generations to _versions
...
정본. 거기에 프롤로그·통제문서·2부가 더해져 수가 늘었다. **지금 읽는 정본은 아래
"현행 정본"뿐이고, 나머지는 이전 세대·영어 카운터파트·작업 통제문서다.**
...
이전 세대(003-010 등)는 이후 `_versions/`로 옮겨 이 목록을 정본만 남길 예정이다.
...
기준. 편집 정본은 draft, 읽기 산출물은 [40_release/002](../40_release/002_first_arc_manuscript_ko.md).
