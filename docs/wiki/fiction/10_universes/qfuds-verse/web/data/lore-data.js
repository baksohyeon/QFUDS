...
window.QFUDS_LORE = (function () {
  ...
  var nodes = [
    ...
    {
      id: 'restoration-copy', cat: 'term', era: 'age1', title: '복원 = 사본', sub: '정본 기전 (021)',
    ...
    // ───────────── SAGA 부 구조 (saga) ─────────────
    {
      ...
      body: '<p>SAGA의 씨앗이 되는 부. 무대는 근미래(2026-2040s) — 먼 미래는 도달점이지 무대가 아니다(근미래 리센터, 025).</p><p>캐스의 시대는 감각이 무너지는 시대다. 얼굴, 목소리, 사진, 영상, 신원이 흔들린다. 이때의 공포는 기술 설명이 아니라 생활감이다: 엄마에게서 온 영상통화가 진짜인지 모른다. 내 얼굴이 내가 하지 않은 말에 붙는다. 내 목소리로 계약이 체결된다.</p><p>캐스의 질문: <b>「나는 나다」를 누가 증명해 주는가.</b></p>',
    ...
    },
    {
      ...
      body: '<p>Q-Day 직후를 다루는 부(035, 작성 중). 주역은 오웬(원본)과 리브(오웬의 어머니, 복원 사본).</p><p>오르페우스 구조에서 「돌아봄」은 신화 모티프가 아니라 <b>검증 행위</b>다. 죽은 사람을 다시 보려는 욕망과, 그 사람이 정말 그 사람인지 확인하려는 폭력이 겹친다.</p><p>오웬의 질문: <b>사랑한다면 확인해야 하는가, 아니면 확인하지 말아야 하는가.</b></p>',
      links: ['owen', 'liv']
    },
    {
  ...
  var qaContext = [
    ...
    '- Q-Day 연표 드리프트: 딥타임 척추(039)는 Q-Day를 먼 미래에 두고, SAGA 부 좌표(024)는 사엘의 Q-Day를 21세기 말에 둔다. 문서 간 충돌이 실재하며 원칙은 「충돌 시 024 우선」. 이건 독자의 오해가 아니라 실제 정리 중인 드리프트다.',
    ...
    '- 파국 기둥 사슬(040): 기둥1 사법·신원 임계 붕괴(디지털 진짜 추정의 사망→검증 노동 탄생) → 기둥2 검증 대공황(미검증=사회적 죽음) → 기둥3 암호 최후 앵커 성역화(제네시스 체인=최후의 공증인, PQC 이주하나 ECDSA 과거는 소급 교체 불가) → 기둥4 Q-Day.',
    ...
    '- 심층시간(036): 5기 관측 전쟁(진짜 복원 주장→예방 전쟁, 마지막 국가 간 전쟁, QFUDS 금지) → 응고기(Last Archive가 은하 중심 블랙홀과 한 몸, Ultima Statio) → 6기 망각권 혁명(실패→전설, 망각조차 계급, Tombstone, 300년 뒤 지도자들이 AI 학습 데이터에서 발견) → 7기 네 도장 교리화(절차의 신성화) → 8기 베일 분열(적색/흑색/회색, 「우리가 지키는 것은 동의다」) → 생물 표류(Archive가 진화를 동결, 표류 후손) → 9기 접근(스파이스 누수, 마지막 백업: 다음 우주의 초기 조건으로 압축).',
    ...
    '',
    '## 확장 레지스터 (030-035, 전부 「후보·보류」 — 캐논 아님, 원고가 당길 때 개별 승격)',
    ...
    '',
    '## 14도메인 세계-체계 매트릭스 (028, 026 부속 — 붕괴의 배선도)',
  ...
  var promoContext = qaContext + '\n\n' + [
    ...
    '- 충돌 우선순위 명시: 021(복원=사본) > 002/010/011 > 024(캐릭터 좌표) > 나머지. 새 설정이 기존 캐논과 부딪히면 지어내지 말고 「충돌 주의」로 표기.',
    ...
    '',
    '## 원칙',
    '- 후보(cand)를 캐논으로 올릴 때, 근거 없는 새 고유명·새 사건을 창작하지 않는다. 기존 후보 레지스터(030-035)에 이미 있는 이름만 정리·승격한다.',
    '- 미정(tbd)은 메우되, 캐논이 의도적으로 비운 공백(Karvath Wound의 「왜」 등)은 그대로 존중하고 「의도된 공백 유지」로 표기.',
    '- 사용자가 「연대기 5-9기 보강」처럼 큰 범위를 요청하면, 한 번에 다 쓰지 말고 먼저 어떤 문서로 분할할지(026 계열인지 036 계열인지) 제안하고 하나씩 초안을 낸다.',
