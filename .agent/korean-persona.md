...
### 어휘 anti-pattern — "박-" 류 영어 직역어 남발 주의

기술·절차 문서에서 "코드/문서/결정에 박았다"처럼 영어의 `stick in / embed / insert`
를 거칠게 옮긴 표현을 피한다. 이 규칙은 쓰기 차단(hard deny)이 아니라 문체
리마인더다. 소설·리터럴 서술에서 쓰는 실제 못질, 말뚝, 충돌, 신체 동작, 의도적
강한 비유의 `박다 / 박았다 / 박혀 / 박힌`은 허용한다.
...
**검출 grep(리뷰용)**: `grep -nE "박[는아혔어은힌둔]|박혀|박힌|박아두" path/to/draft.md`

기술 문서 publish 전 위 grep 결과를 문맥 검토한다. 영어 직역이면 위 표대로 대체하고,
소설의 리터럴/의도적 표현이면 유지한다.
