---
doc_id: qfuds_verse_zettelkasten_moc_ko
title: QFUDS Verse 제텔카스텐 MOC
doc_type: index
stage: reference
status: draft
evidence_role: provenance
depends_on:
  - qfuds_verse_zettelkasten_pilot_index_ko
next_gate: expand only after source-linked cards pass validation and fidelity review
last_updated: 2026-07-08
---

# QFUDS Verse 제텔카스텐 MOC

```text
fiction/provenance only
research evidence: no
canon action: navigation only
```

## 목적

이 문서는 Obsidian에서 제텔카스텐 카드를 주제별로 들어가기 위한 Map of Content다.
새 canon을 만들지 않고, 이미 출처를 가진 카드와 원문 문서를 연결한다.

## 시작점

| 필요 | 열 문서 |
| --- | --- |
| 파일럿 전체 | [QFUDS Verse 제텔카스텐 파일럿 인덱스](004_zettelkasten_pilot_index_ko.md) |
| 아이디어 덤프 처리 | [QFUDS Verse 제텔카스텐 아이디어 인박스](006_zettelkasten_inbox_ko.md) |
| 스토리 사용 연결 | [QFUDS SAGA 제텔카스텐 스토리 씨앗 레지스터](../10_universes/qfuds-verse/20_series/qfuds-saga/10_story_design/326_zettelkasten_story_seed_register_ko.md) |

## 복원·죽음

| 카드 | 쓰임 |
| --- | --- |
| [복원은 부활이 아니라 데이터 사본이다](../10_universes/qfuds-verse/10_world/900_zettel_restoration_is_copy_ko.md) | 원본 귀환 금지선 |
| [열역학은 복원의 적이 아니라 비용 구조다](../10_universes/qfuds-verse/10_world/901_zettel_thermodynamic_cost_of_restoration_ko.md) | 비용·계급·손실 |
| [유니터리는 실용적 복원을 보증하지 않는다](../10_universes/qfuds-verse/10_world/905_zettel_unitarity_does_not_guarantee_restoration_ko.md) | 작중 형이상학과 실제 주장 분리 |
| [해상도 계급은 죽음의 계급화다](../10_universes/qfuds-verse/10_world/906_zettel_resolution_class_after_death_ko.md) | 사본 품질과 권력 |
| [위조의 완성이 가치 축을 진위에서 복원가능성으로 옮긴다](../10_universes/qfuds-verse/10_world/919_zettel_forgery_shifts_value_to_recoverability_ko.md) | 진위→복원가능성 축 이동 |

## Q-Day·검증권력

| 카드 | 쓰임 |
| --- | --- |
| [인식 위기와 Q-Day는 같은 사건이 아니다](../10_universes/qfuds-verse/10_world/902_zettel_two_crisis_spine_ko.md) | 시대 좌표 |
| [암호는 처음엔 구원이고 나중엔 재앙이다](../10_universes/qfuds-verse/10_world/903_zettel_crypto_anchor_reversal_ko.md) | 암호 앵커 반전 |
| [Q-Day는 유동성 위기가 아니라 인증 위기다](../10_universes/qfuds-verse/10_world/907_zettel_qday_as_authentication_crisis_ko.md) | 붕괴의 정확한 성격 |
| [검증주권은 진본 확정 독점이다](../10_universes/qfuds-verse/10_world/908_zettel_verification_sovereignty_ko.md) | 국가·회사·법정 권력 |
| [복원 가능한 것은 청구 가능하다는 폭력적 문법이다](../10_universes/qfuds-verse/10_world/909_zettel_recoverable_claimable_grammar_ko.md) | 가능성에서 청구권으로 미끄러지는 규칙 |
| [Q-Day 첫 패닉은 부호가 뒤집힌다](../10_universes/qfuds-verse/10_world/917_zettel_qday_panic_sign_flip_ko.md) | 먼저 움직이는 자가 진다(catch-22) |
| [가장 안전한 자는 가장 가난한 자다](../10_universes/qfuds-verse/10_world/918_zettel_safest_is_poorest_ko.md) | 재난이 부의 사다리를 안 따른다 |
| [명목 단위는 남고 실질 부는 소각된다](../10_universes/qfuds-verse/10_world/922_zettel_nominal_preserved_real_burned_ko.md) | 움직일 수 없는 부=묘비 |
| [진위런의 종착은 실물이다](../10_universes/qfuds-verse/10_world/923_zettel_flight_to_physicality_possession_ko.md) | 점유가 다시 소유가 된다 |

## 권위·불확실성

| 카드 | 쓰임 |
| --- | --- |
| [캐논 권위는 도메인별로 갈린다](../10_universes/qfuds-verse/00_continuity/910_zettel_domain_authority_map_ko.md) | 충돌 해소 |
| [하드 연도보다 소프트 시대 좌표가 우선한다](../10_universes/qfuds-verse/00_continuity/911_zettel_soft_chronology_policy_ko.md) | 연표 추정 방지 |
| [심층시간은 확장 레이어이며 상위 가드를 넘지 않는다](../10_universes/qfuds-verse/00_continuity/912_zettel_deep_time_extension_guard_ko.md) | 003 사용 가드 |
| [불명확한 것은 unknown으로 남긴다](../10_universes/qfuds-verse/00_continuity/913_zettel_unknown_as_authorial_tool_ko.md) | 추정 금지 |
| [라우어의 네 도장은 불확실성 보존 장치다](../10_universes/qfuds-verse/00_continuity/914_zettel_laur_four_stamps_uncertainty_ko.md) | defined/assumed/unknown/circular_if_fitted |
| [상실의 저자는 누구인가](../10_universes/qfuds-verse/10_world/920_zettel_who_may_author_loss_top_theme_ko.md) | 사가 최상위 테마 |
| [마지막 반박권은 한 손에 모이지 않는다](../10_universes/qfuds-verse/10_world/921_zettel_last_refutation_never_permanent_ko.md) | 마찰이 세계를 굴린다 |
| [Last Archive는 지성이 아니라 합의의 신격화다](../10_universes/qfuds-verse/10_world/961_zettel_agi_is_artificial_general_agreement_ko.md) | AGI 아님, 반박 멈춘 합의 |

## 세력·권력 구조

| 카드 | 쓰임 |
| --- | --- |
| [삼권분립은 안정이 아니라 네 긴장 위의 임시 균형이다](../10_universes/qfuds-verse/10_world/948_zettel_three_powers_stand_on_four_tensions_ko.md) | 세력 균형의 불안정성 |
| [exit는 떠날 수 있는 자에게만 있는 자유다](../10_universes/qfuds-verse/10_world/949_zettel_exit_is_freedom_only_for_those_who_can_leave_ko.md) | exit>voice의 계급성 |

## 전시·증명 경계

| 카드 | 쓰임 |
| --- | --- |
| [전시물은 증명이 아니라 물려받은 실패의 유물이다](../10_universes/qfuds-verse/10_world/915_zettel_exhibit_is_inherited_failure_not_proof_ko.md) | 시각 전시물의 fiction/provenance 가드 |

## 크래프트·서술

| 카드 | 쓰임 |
| --- | --- |
| [개념은 직접 쓰고 풀어 설명한다 — 비유로 숨기지 않는다](../10_universes/qfuds-verse/00_continuity/916_zettel_concept_first_no_metaphor_hiding_ko.md) | 용어 직접·비유 과잉 금지 |
| [기본 시점은 3인칭 제한, 1인칭은 증거물로만 쓴다](../10_universes/qfuds-verse/00_continuity/974_zettel_third_limited_default_first_person_as_evidence_ko.md) | 시점 규칙 |
| [고유명사는 통용명과 의례/법정명을 겹쳐 둔다](../10_universes/qfuds-verse/00_continuity/975_zettel_two_layer_naming_common_formal_ko.md) | 명명 규칙 |

## 확장 규칙

새 카드는 먼저 원문 source ref를 가져야 한다. 아이디어만 있는 항목은 inbox에 두고,
원문·story design·chronicler pass 중 하나로 출처를 얻기 전까지 zettel로 승격하지 않는다.
