---
doc_id: qfuds_verse_zettel_thermodynamic_cost_of_restoration_ko
title: 열역학은 복원의 적이 아니라 비용 구조다
doc_type: reference
stage: reference
status: draft
evidence_role: provenance
depends_on:
  - qfuds_saga_restoration_mechanism_correction_ko
next_gate: keep restoration cost language aligned with 113 before drafting restoration scenes
last_updated: 2026-07-07
---

# 열역학은 복원의 적이 아니라 비용 구조다

```text
fiction/provenance only
research evidence: no
canon_state: canon summary of 113, not a new claim
```

## Statement

복원은 엔트로피를 되돌리는 일이 아니라, 남은 데이터를 처리해 사본을 짓는 전진 계산이며
그 대가로 에너지·손실·계급 비용을 만든다.

## Source Refs

- [113 §4.5](../../../../docs/wiki/fiction/10_universes/qfuds-verse/10_world/113_restoration_mechanism_correction_ko.md#45-열역학은-절대다-유니터리와-양립-검증): 제2법칙과 유니터리의 혼동을 금지한다.
- [113 §4.6](../../../../docs/wiki/fiction/10_universes/qfuds-verse/10_world/113_restoration_mechanism_correction_ko.md#46-복원의-정확한-열역학): 복원은 전진 계산이며 막대한 엔트로피 비용을 만든다고 정리한다.

## Context

복원의 비용은 세계관 장식이 아니다. 복원 충실도, 반복 복원 손실, 부유층의 더 높은
해상도 구매, 약자의 망각권 박탈을 함께 밀어 올리는 구조다. 물리 불가능성을 숨기는
비유가 아니라, 복원이 왜 시장과 법정의 문제가 되는지 설명하는 압력이다.

## Links

- [복원은 부활이 아니라 데이터 사본이다](900_zettel_restoration_is_copy_ko.md)
- [불명확한 것은 unknown으로 남긴다](../00_continuity/913_zettel_unknown_as_authorial_tool_ko.md)
- [QFUDS SAGA 복원 메커니즘 정정](../../../../docs/wiki/fiction/10_universes/qfuds-verse/10_world/113_restoration_mechanism_correction_ko.md)

## Open Questions

- 복원 비용이 특정 장면에서 돈, 에너지 배급, 법정 수수료, 사회 계급 중 무엇으로 보일지 원고별 선택이 필요하다.

## Use Guard

외부 물리 주장을 새로 추가하지 않는다. 이 카드는 113의 작중 적용을 돕는 요약이다.
